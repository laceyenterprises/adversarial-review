...
const DEFAULT_REVIEW_LANE_SHARE_SUPERMAJORITY_DISTINCT_PR_FLOOR = 2;
...
const REVIEW_PIPELINE_HEALTH_METRICS = Object.freeze([
  ...
  'review_pipeline_first_pass_ci_orphans_collected',
...
const REVIEW_PIPELINE_HEALTH_METRIC_HELP = Object.freeze({
  ...
  review_pipeline_first_pass_ci_orphans_collected: 'Whether the first-pass CI orphan collector probed every candidate successfully.',
...
const REVIEW_PIPELINE_HEALTH_FINDING_DEFINITIONS = Object.freeze([
  ...
  },
  {
    ...
    thresholdKey: null,
    defaultThreshold: null,
    thresholdDescription: 'an open first-pass PR has failing checks, no verdict, and no known live worker on its branch',
  },
  {
    code: 'review:first_pass_ci_orphan_probe_blind',
    tier: 'ticket',
    category: 'review-pipeline',
    thresholdKey: 'firstPassCiOrphanDeadlineMs',
    defaultThreshold: DEFAULT_FIRST_PASS_CI_ORPHAN_DEADLINE_MS,
    thresholdDescription: 'the first-pass CI orphan collector could not probe every candidate successfully',
  },
  {
...
function resolveReviewPipelineHealthConfig(env = process.env, overrides = {}) {
  ...
  return {
    ...
    reviewLaneShareSupermajorityDistinctPrFloor: parseIntegerAtLeast(
      overrides.reviewLaneShareSupermajorityDistinctPrFloor
        ?? env.ADVERSARIAL_REVIEW_PIPELINE_HEALTH_LANE_SHARE_SUPERMAJORITY_DISTINCT_PR_FLOOR,
      DEFAULT_REVIEW_LANE_SHARE_SUPERMAJORITY_DISTINCT_PR_FLOOR,
      1
    ),
    ...
    firstPassCiOrphanMaxProbedPrs: parsePositiveInteger(
      overrides.firstPassCiOrphanMaxProbedPrs
        ?? env.ADVERSARIAL_REVIEW_PIPELINE_HEALTH_FIRST_PASS_CI_ORPHAN_MAX_PROBED_PRS,
      DEFAULT_FIRST_PASS_CI_ORPHAN_MAX_PROBED_PRS
    ),
    firstPassCiOrphanDeadlineMs: parsePositiveInteger(
      overrides.firstPassCiOrphanDeadlineMs
        ?? env.ADVERSARIAL_REVIEW_PIPELINE_HEALTH_FIRST_PASS_CI_ORPHAN_DEADLINE_MS,
      DEFAULT_FIRST_PASS_CI_ORPHAN_DEADLINE_MS
    ),
    firstPassCiOrphanWorkerStatusTimeoutMs: parsePositiveInteger(
      overrides.firstPassCiOrphanWorkerStatusTimeoutMs
        ?? env.ADVERSARIAL_REVIEW_PIPELINE_HEALTH_FIRST_PASS_CI_ORPHAN_WORKER_STATUS_TIMEOUT_MS,
      DEFAULT_FIRST_PASS_CI_ORPHAN_WORKER_STATUS_TIMEOUT_MS
    ),
...
function summarizeFirstPassQueue(db, { nowMs, stoppedCiRegressionJobs = null }) {
  ...
  for (const row of rows) {
    ...
    if (watcherPassKind === 'first-pass') {
      ...
      firstPassPrs.push(firstPassPr);
      if (!oldestFirstPass || pendingAgeMs > oldestFirstPass.ageMs) {
        oldestFirstPass = firstPassPr;
      }
    }
  }
  return {
...
function failedCheckNames(rollup) {
  ...
  for (const check of Array.isArray(rollup) ? rollup : []) {
    ...
    if (bucket && !['fail', 'cancel'].includes(bucket)) continue;
    if (bucket && ['fail', 'cancel'].includes(bucket)) {
...
function workerManifestMatches(manifests, repo, branch) {
  ...
  return repos.size === 0 || repos.has(repo) || repos.has(bareRepoName(repo));
}
...
function branchWorkerIndex(hqRoot) {
  const workersDir = join(hqRoot, 'workers');
  const workers = [];
  if (!existsSync(workersDir)) return workers;
  for (const workerId of readdirSync(workersDir)) {
    const manifests = readWorkerManifests(join(workersDir, workerId));
    workers.push({
      workerId,
      manifests,
      launchRequestId: workerLaunchRequestId(manifests),
    });
  }
  return workers;
}

function liveWorkerForBranch(hqRoot, repo, branch, {
  execFileSyncImpl,
  workerIndex = branchWorkerIndex(hqRoot),
  workerStatusTimeoutMs = DEFAULT_FIRST_PASS_CI_ORPHAN_WORKER_STATUS_TIMEOUT_MS,
}) {
  if (!branch) return null;
  const hqBin = process.env.AGENT_OS_HQ_BIN || process.env.HQ_BIN || 'hq';
  const ownerUser = readHqOwnerUser(hqRoot);
  const liveStatuses = new Set(['requested', 'admitted', 'leased', 'starting', 'running']);
  for (const { workerId, manifests, launchRequestId } of workerIndex) {
    if (!workerManifestMatches(manifests, repo, branch)) continue;
    if (!launchRequestId) continue;
    try {
      const args = ['dispatch', 'status', launchRequestId, '--json'];
      if (ownerUser) args.push('--as-owner', ownerUser);
      const output = execFileSyncImpl(
        hqBin, args,
        { encoding: 'utf8', timeout: workerStatusTimeoutMs, stdio: ['ignore', 'pipe', 'pipe'] }
      );
      const status = String(parseJson(output, {})?.status || '').toLowerCase();
      // Keep in sync with session-ledger launch_request_terminal_statuses.py:
      // only known non-terminal statuses suppress this orphan diagnostic.
...
function execGhWithTransientRetry(args, {
  execFileSyncImpl,
  sleepSyncImpl = sleepSyncMs,
  timeout = 20_000,
  acceptStdoutOnError = false,
}) {
  const options = {
    encoding: 'utf8',
    timeout,
  ...
  for (let attempt = 0; attempt <= DEFAULT_GH_TERMINAL_STATE_RETRY_DELAYS_MS.length; attempt += 1) {
    try {
      ...
      if (acceptStdoutOnError && parseJsonArray(stdout).length > 0) return stdout;
      lastError = error;
...
function summarizeFirstPassCiOrphans(firstPassQueue, {
  ...
  const prs = [];
  const errors = [];
  const candidates = (firstPassQueue.firstPassPrs || [])
    .filter((queued) => queued.reviewAttempts <= 0 && !queued.reviewerFailed);
  const maxProbedPrs = config.firstPassCiOrphanMaxProbedPrs || DEFAULT_FIRST_PASS_CI_ORPHAN_MAX_PROBED_PRS;
  const deadlineMs = config.firstPassCiOrphanDeadlineMs || DEFAULT_FIRST_PASS_CI_ORPHAN_DEADLINE_MS;
  const deadlineAt = Date.now() + deadlineMs;
  const workerIndex = branchWorkerIndex(hqRoot);
  let probedPrs = 0;
  let unprobedPrs = 0;
  for (const queued of candidates) {
    if (probedPrs >= maxProbedPrs || Date.now() > deadlineAt) {
      unprobedPrs += 1;
      continue;
    }
    probedPrs += 1;
    ...
    try {
      const output = execGhWithTransientRetry(
        ['pr', 'view', String(queued.prNumber), '--repo', queued.repo, '--json',
          'state,headRefName,headRefOid'],
        { execFileSyncImpl, sleepSyncImpl }
      );
      const pr = parseJson(output, {});
      const checksOutput = execGhWithTransientRetry(
        ['pr', 'checks', String(queued.prNumber), '--repo', queued.repo, '--json', 'name,state,bucket'],
        { execFileSyncImpl, sleepSyncImpl, acceptStdoutOnError: true }
      );
      ...
      const liveWorker = liveWorkerForBranch(hqRoot, queued.repo, pr.headRefName, {
        execFileSyncImpl,
        workerIndex,
        workerStatusTimeoutMs: config.firstPassCiOrphanWorkerStatusTimeoutMs,
      });
      if (liveWorker?.error) {
  ...
  return {
    count: prs.length,
    collected: errors.length === 0 && unprobedPrs === 0,
    probedPrs,
    unprobedPrs,
...
function activeFollowUpJobsByPr(followUpJobs) {
  ...
  for (const entry of followUpJobs || []) {
    ...
    const existing = activeJobsByPr.get(key);
    if (!existing || entry.state === 'in_progress') {
...
function summarizeDeferredRereviews(db, followUpJobs, { nowMs, stoppedCiRegressionJobs = null }) {
  ...
  prs.sort((left, right) => (right.ageMs || 0) - (left.ageMs || 0));
...
function jobOperationalBlockers(job) {
  const sources = [
    ...
    job?.remediationReply?.operationalBlockers,
    job?.completion?.parsedReply?.operationalBlockers,
    job?.failure?.operationalBlockers,
    job?.operationalBlockers,
  ];
  const blockers = [];
  for (const source of sources) {
    if (Array.isArray(source)) blockers.push(...source);
  }
  return blockers;
}

function jobRequiresHumanIntervention(job, blockers) {
  const texts = [
    job?.remediationPlan?.stop?.reason,
    job?.stopReason,
    job?.failure?.message,
    job?.commentDelivery?.body,
    ...blockers.flatMap((blocker) => (
      blocker && typeof blocker === 'object' && !Array.isArray(blocker)
        ? [blocker.finding, blocker.reasoning, blocker.needsHumanInput]
        : [blocker]
    )),
...
function summarizeOperationalBlockers(followUpJobs, { nowMs }) {
  ...
  for (const entry of followUpJobs) {
    if (entry.state !== 'stopped') continue;
    const blockers = jobOperationalBlockers(entry.job);
    if (!blockers.length) continue;
    const stoppedAt = entry.job.stoppedAt
      || entry.job.remediationPlan?.stop?.stoppedAt
      || terminalJobTimestamp(entry.job, entry.stat.mtimeMs);
    const stoppedAgeMs = ageMs(nowMs, stoppedAt);
    const pr = {
      repo: entry.job.repo || null,
      prNumber: Number(entry.job.prNumber) || null,
      jobId: entry.job.jobId || null,
      stoppedAt,
      ageMs: stoppedAgeMs,
      rescue: entry.job.operationalBlockerRecovery?.rescue || null,
    };
    const humanInterventionRequired = jobRequiresHumanIntervention(entry.job, blockers);
    for (const blocker of blockers) {
      const category = normalizeOperationalBlockerCategory(blocker);
      ...
      const item = {
        ...pr,
        category,
        finding: typeof blocker === 'object' && blocker !== null ? blocker.finding || null : String(blocker || ''),
        humanInterventionRequired,
      };
      summary.prs.push(item);
      if (stoppedAgeMs !== null && (!summary.oldest || stoppedAgeMs > summary.oldest.ageMs)) {
        summary.oldest = item;
      }
      byCategory.set(category, summary);
      rounds.push(item);
    }
  }
  return {
...
function evaluateReviewPipelineFindings(snapshot, { observedAt }) {
  ...
  if (
    totalLanePasses >= config.reviewLaneShareSupermajorityMinPasses
    && Number(capacity.effectiveConcurrency || 0) > 1
  ) {
    ...
    if (dominantLane) {
      ...
      const starvedLanePrs = starvedLane === 'first-pass'
        ? (snapshot.firstPassQueue.firstPassPrs || [])
        : (snapshot.queuedRereviews.prs || []);
      const distinctQueuedPrs = new Set(
        starvedLanePrs.map((pr) => `${pr.repo}#${pr.prNumber}`)
      ).size;
      const queuedAgeMs = Number(queuedEvidence?.ageMs || 0);
      if (
        queuedEvidence
        && queuedAgeMs >= config.queueStarvationMaxAgeMs
        && distinctQueuedPrs >= config.reviewLaneShareSupermajorityDistinctPrFloor
      ) {
        findings.push(buildFinding({
          code: 'review:review_lane_share_supermajority',
          tier: 'ticket',
          ...
          evidence: [
            ...
            `queued_${starvedLane.replace('-', '_')}_distinct_prs=${distinctQueuedPrs} `
            + `distinct_pr_floor=${config.reviewLaneShareSupermajorityDistinctPrFloor} `
          ...
          details: {
            ...
            distinctPrFloor: config.reviewLaneShareSupermajorityDistinctPrFloor,
            ...
            distinctQueuedPrs,
            queuedAgeThresholdMs: config.queueStarvationMaxAgeMs,
  ...
  if (
    snapshot.firstPassCiOrphans
    && snapshot.firstPassCiOrphans.collected === false
    && (
      (snapshot.firstPassCiOrphans.errors || []).length > 0
      || Number(snapshot.firstPassCiOrphans.unprobedPrs || 0) > 0
    )
  ) {
    findings.push(buildFinding({
      code: 'review:first_pass_ci_orphan_probe_blind',
      tier: 'ticket',
      subject: 'First-pass CI orphan probe is blind',
      message: 'GitHub checks, worker-liveness probes, or the configured first-pass CI orphan budget prevented a complete snapshot; this is not proof that no PR heads are orphaned.',
      evidence: (snapshot.firstPassCiOrphans.errors || []).slice(0, 3),
      recommendedAction: 'Inspect the probe errors or raise the first-pass CI orphan probe budget before treating the orphan count as clean.',
      observedAt,
      details: snapshot.firstPassCiOrphans,
    }));
  }
  ...
  for (const orphan of snapshot.firstPassCiOrphans?.prs || []) {
    findings.push(buildFinding({
      ...
      message: `Head ${orphan.headSha} fails check(s) ${orphan.failedChecks.join(', ')}; no review verdict or known live worker owns branch ${orphan.headRefName}. First-pass review is not CI-gated, so this diagnostic is additive to queue-starvation findings.`,
      evidence: [
        `gh pr view ${orphan.prNumber} --repo ${orphan.repo}`,
        ...(snapshot.firstPassCiOrphans.errors || []).slice(0, 3),
      ],
  ...
  const deferredRereviewKeys = new Set(
    (snapshot.deferredRereviews.prs || []).map((pr) => `${pr.repo}#${pr.prNumber}`)
  );
  const waitingRereviews = (snapshot.queuedRereviews.prs || [])
    .filter((pr) => !deferredRereviewKeys.has(`${pr.repo}#${pr.prNumber}`));
  const oldestWaitingRereview = waitingRereviews
    .slice()
    .sort((left, right) => (right.ageMs || 0) - (left.ageMs || 0))[0] || null;
  if (oldestWaitingRereview && oldestWaitingRereview.ageMs > config.queueStarvationMaxAgeMs) {
    findings.push(buildFinding({
      code: 'review:rereview_queue_wait',
      tier: 'ticket',
      subject:
        `${waitingRereviews.length} re-review(s) awaiting reviewer admission; oldest is `
        + `${Math.round(oldestWaitingRereview.ageMs / 60000)}m old`,
      message:
      ...
      recommendedAction:
        'This is a re-review lane wait, not first-pass starvation. Check the watcher claim CAS, '
        + 'reviewer lane allocation, dispatch drain state, and reviewer-specific capacity caps; if '
        + 'there is an intentional hold, record the deferral reason instead of guessing.',
      observedAt,
      details: {
  ...
  const operationalBlockerRounds = (snapshot.operationalBlockers?.rounds || [])
    .filter((round) => round.humanInterventionRequired);
  if (operationalBlockerRounds.length > 0) {
    const sample = operationalBlockerRounds
      .slice()
      .sort((left, right) => (right.ageMs || 0) - (left.ageMs || 0))[0];
    findings.push(buildFinding({
      code: 'review:operational_blocker_human_intervention',
      tier: 'ticket',
      subject: `${operationalBlockerRounds.length} stopped remediation round(s) require human intervention`,
      message: `${sample.repo}#${sample.prNumber} stopped with operational blocker ${sample.category}; oldest age ${Math.round((sample.ageMs || 0) / 60000)}m.`,
      evidence: operationalBlockerRounds.map((round) => (
        `follow-up-jobs/stopped ${round.repo}#${round.prNumber} category=${round.category} stoppedAt=${round.stoppedAt || 'unknown'}`
      )),
      recommendedAction: 'Inspect the stopped job operationalBlockers[] payload and any recorded rescue ref/bundle, repair the named operational fault, then retrigger from preserved work instead of rerunning the round from scratch.',
      observedAt,
...
function collectReviewPipelineHealth({
  ...
  try {
    ...
    const firstPassCiOrphans = config.hostChecksEnabled
      ? summarizeFirstPassCiOrphans(firstPassQueue, {
          hqRoot,
          execFileSyncImpl,
          sleepSyncImpl,
          config,
      ...
      : { count: 0, collected: false, probedPrs: 0, unprobedPrs: 0, totalCandidates: 0, prs: [], errors: [] };
...
function renderReviewPipelinePrometheus(snapshot) {
  ...
  pushMetric(
    'review_pipeline_first_pass_ci_orphans_collected',
    {},
    snapshot.firstPassCiOrphans?.collected ? 1 : 0
  );
  ...
  for (const row of operationalBlockerCategories) {
    pushMetric('review_pipeline_operational_blocker_rounds', {
      category: row.category,
    }, row.count);
    pushMetric('review_pipeline_operational_blocker_oldest_age_seconds', {
      category: row.category,
    }, Math.round((row.oldest?.ageMs || 0) / 1000));
  }
