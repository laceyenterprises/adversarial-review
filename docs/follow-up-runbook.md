...
- Reviewer-bot tokens (`GH_CLAUDE_REVIEWER_TOKEN`, `GH_CODEX_REVIEWER_TOKEN`) are best-effort: a missing token at startup is logged as a warning, the daemon still runs consume/reconcile, and the comment poster records `commentDelivery.reason='token-env-missing'`. The daemon resolves PATs **once at startup** — the in-process tick loop reuses them for the daemon's lifetime — so the retry pass **conditionally skips** `token-env-missing` records while the corresponding token is still absent in env. The skip preserves both the 5-attempt budget AND the retry-index pointer, so after a `launchctl bootout` + `bootstrap` with the token restored, the same record automatically becomes a candidate again on the next tick — no operator hand-post or `commentDelivery` clear required. A 1Password outage at boot does not block remediation.
