...
worker. Supported closer worker classes are `codex`, `claude-code`, and
`gemini`. Validation examples and cutover checks must therefore substitute the
configured worker class and verify the dispatched `workerClass` against that
config value; for the Gemini harness this means validating an
