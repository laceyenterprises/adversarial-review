...
- The optional `run-local-review-shadow` PR label requests a non-gating local model shadow review after the hosted adversarial review posts. Configure the local model with `ADVERSARIAL_LOCAL_REVIEW_SHADOW_MODEL` or `LOCAL_REVIEW_SHADOW_MODEL`; configure the LiteLLM chat-completions endpoint with `ADVERSARIAL_LOCAL_REVIEW_SHADOW_URL` or `LOCAL_REVIEW_SHADOW_URL` (default `http://127.0.0.1:4000/v1/chat/completions`); set `ADVERSARIAL_LOCAL_REVIEW_SHADOW_TIMEOUT_MS` or `LOCAL_REVIEW_SHADOW_TIMEOUT_MS` to override the default `90000ms` call timeout; and set `ADVERSARIAL_LOCAL_REVIEW_SHADOW_API_KEY`, `LOCAL_REVIEW_SHADOW_API_KEY`, or `LITELLM_API_KEY` only if the local proxy requires a bearer token. The endpoint is fail-closed to loopback HTTP(S) hosts (`localhost`, `127/8`, or `::1`) before the PR diff or hosted review body is sent. Remote URLs are recorded as retryable local configuration failures, not contacted.
- Local shadow state is durable under `data/local-review-shadow/`: `requests/*.json` records status, eligibility, attempts, backoff, and error details; `inputs/*.json` stores the diff and hosted review body only for eligible requests; and `artifacts/*.md` stores the non-gating local output. Files are written with mode `0600`. Ineligible requests are `skipped` and do not persist private inputs. Retryable failures back off for 15 minutes via `nextAttemptAt`; posted-review reconciliation starts due work in the watcher background and does not block the hosted review, gate projection, or merge-agent path. To recover, fix the local LiteLLM/model/env issue and leave the label in place for the next due retry; remove the label to stop new starts, and inspect the request JSON `lastError` for diagnosis.
