...
- `operator-approved` is the operator's current-head merge approval when they have decided the substance is fine. The watcher resolves the latest matching GitHub `labeled` event from the PR timeline and accepts the label only when that event is attributable and occurs after the timeline code event for the current head SHA. At single-operator scale the label remains valid even when the actor and PR author login match; the head-scope and timing checks are the real freshness gates. The label can bypass review/remediation-state gates, but it does NOT bypass closed/merged PRs, not-mergeable state, failed, pending, or unknown CI, or explicit skip labels.
- `merge-agent-requested` is a stronger one-shot operator request to dispatch merge-agent so it can clean or rebase a stuck branch. The watcher resolves the latest matching GitHub `labeled` event and accepts the label only when that event is attributable and scoped to the current head SHA; stale or unattributed label events produce `skip-merge-agent-requested-stale`. When scoped, the label can bypass missing/unknown verdicts, current mergeability, failed or pending checks, remediation-round gates, and the terminal `merge-agent-stuck` handoff marker for the same head, but it does NOT bypass closed/merged PRs, active remediation, `merge-agent-skip`, `do-not-merge`, or duplicate dispatch protection.
- `merge-agent-dispatched` is a watcher-owned lifecycle label, not an operator trigger. The watcher adds it after a successful merge-agent dispatch so operators can see that a merge worker is out for the PR and so the merge/close cleanup path has a public retry marker. If label application fails after dispatch, the watcher must persist a retryable lifecycle cleanup record so the dispatched-but-unlabeled state is recoverable.
- `merge-agent-skip` and `do-not-merge` are unbypassable explicit skip labels. `merge-agent-stuck` is a terminal watcher handoff marker that normally returns `skip-operator-skip`, but a scoped current-head `merge-agent-requested` label is the documented operator recovery action for that one state.
- Merge-agent dispatch precedence is: closed/merged PRs; unbypassable skip labels; scoped `operator-approved` hard gates; active remediation; the normal verdict/mergeable/check/remediation gates; scoped `merge-agent-requested` override for bypassable gates and `merge-agent-stuck` recovery; stale `operator-approved` diagnostics only when no dispatch path applies; duplicate-dispatch protection; dispatch. When a scoped `merge-agent-requested` label is present on a PR that the normal path would already dispatch without an operator trigger, the request label is treated as the authorizing trigger so the one-shot operator request is consumed. A successful dispatch removes only the label that actually authorized that dispatch, preserving unused operator labels as audit trail. If the PR later merges or closes while `merge-agent-dispatched` is still present or a matching merge-agent dispatch record exists, the watcher must durably queue lifecycle cleanup before the PR leaves the open-set query, retry `hq dispatch cancel` plus label removal on later ticks until they reach terminal outcomes, and keep `merge-agent-dispatched` present after non-terminal cancel failures so the recovery signal is not lost. Pending lifecycle cleanups must be paced and capped so retry storms do not block normal watcher work, and malformed cleanup records must be surfaced loudly.
