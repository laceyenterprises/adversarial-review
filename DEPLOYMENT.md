...
These files embed local paths and account names. They are examples of
**one** deployment topology — the maintainer's — not prerequisites for
the demo or test suite. Outside operators are expected to write their
own launchd / systemd unit files (or use the templates under
`tools/adversarial-review/deploy/`).
