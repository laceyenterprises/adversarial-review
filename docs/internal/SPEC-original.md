...
- `merge-agent-dispatched` is a watcher-owned lifecycle label, not an operator trigger. The watcher adds it after a successful merge-agent dispatch so operators can see that a merge worker is out for the PR and so the merge/close cleanup path has a public retry marker.
