...
For `review_status='failed'`, the watcher may auto-retry only through the normal review dispatch path. The row must be rediscovered as a current open PR in an active repo, watcher drain must be inactive, active follow-up/backoff/merge-agent handoff gates must allow the spawn, memory admission must pass, routing-tier readiness must be healthy, and the SQL compare-and-swap must promote the row to `reviewing`. The prior `failed_at` and `failure_message` remain intact until that claim succeeds. Infrastructure-class failures (`cascade`, `reviewer-timeout`, `launchctl-bootstrap`, and reviewer-spawn `oauth-broken`) additionally increment `infra_auto_recover_attempts` after the successful claim; once that counter reaches the cap, the row remains `failed` for operator inspection. `forbidden-fallback`, inactive repos, PRs missing from GitHub discovery, closed or merged PRs, and rows skipped during watcher drain are not silently reset.
