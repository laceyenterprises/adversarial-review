...
- `diagnose-stuck-rereview --apply` is an automation watchdog, not an operator retrigger surface. It scans pending rows with aged `rereview_requested_at`, then preserves that original FIFO/latency timestamp, refreshes only `rereview_reason` with the non-operator `stuck-rereview-watchdog:` prefix, and writes a watcher wake for the affected repo/PR/head so the next watcher tick prioritizes the row without making it look fresh. It does not clear `review_attempts`, `last_attempted_at`, `failed_at`, `failure_message`, `reviewer_head_sha`, or retry counters, and rows carrying `failed_at` stay on the terminal-failure retry/finalization path. Re-arms are tracked in `data/follow-up-jobs/stuck-rereview-watchdog.json` by `(repo, pr, head)`, use exponential backoff from the stuck threshold, record the original request time, prune entries whose last successful apply is older than seven days whenever the state file is rewritten, and stop after three attempts; cap exhaustion is reported as a skipped row rather than an apply failure so a capped row cannot consume the window forever.
