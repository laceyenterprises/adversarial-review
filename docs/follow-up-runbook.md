...
- **Auto-remediation defaults to cross-model (adversarial) routing per PR's `builderTag`** (restored 2026-05-29 PM by PR #175 after PR #172's same-model attempt): `[codex]` PRs → claude-code remediates, `[claude-code]` PRs → codex remediates, `[clio-agent]` PRs → claude-code remediates (Clio dispatches codex writers, so claude-code is the opposite). Both reviewer AND remediator are adversarial-by-default. Operators can override that default with `ADVERSARIAL_REVIEW_DEFAULT_REMEDIATOR=codex|claude-code|gemini`; aliases `claude`, `claude-code-remediation`, `codex-remediation`, and `gemini-remediation` are accepted, while unknown non-empty values are configuration errors. When `builderTag` is missing or unknown (job-schema-migration field loss, hand-edited triage records), the derivation falls through to a degraded same-model fallback to `codex`; the operator can pin via `ADVERSARIAL_REVIEW_DEFAULT_REMEDIATOR` to bypass. The durable `builderTag` is still preserved on the job ledger for downstream queue/audit purposes. Codex remediation commits still use the remediation trailer class configured by the spawn path, and reconcile-time public PR comments post under the selected worker bot identity when the worker model is known; `never-spawned` reconcile paths fall back to the configured/default worker class. Historical note: LAC-358 (March 2026) previously hard-routed all remediation to codex regardless of builderTag — that override has been reverted, restored, reverted, and re-restored over the intervening months; today's default is the cross-model adversarial routing documented above.
- When the routed remediator has a confirmed quota cap, the follow-up consumer checks both Claude and Codex as automatic fallback candidates and selects only a candidate with confirmed available quota. The primary route resumes after its quota recovers. Reconcile reads quota evidence from both the worker log and final-message artifact; a generic account 429 in the artifact is held as `quota-exhausted`, not accepted as a completed remediation narrative.
- Advancing from one remediation round to another runs **automatically** via the `ai.laceyenterprises.adversarial-follow-up` LaunchAgent (long-lived, internal 120s tick loop running entirely in a single node process). On startup the daemon registers App Contract telemetry subscriptions for configured `health.worker.*` topics. In `agent-os` mode this registration is staged until an inbound App Contract topic transport feeds `session.emitTopic(...)`; until then, no production telemetry wakeup is expected and the periodic scanner remains authoritative. Registration is best-effort and bounded by `ADVERSARIAL_REVIEW_TELEMETRY_LISTENER_START_TIMEOUT_MS` (default 5000ms), so a slow broker disables only the listener for that start rather than blocking the tick loop. Once delivery is wired, delivered `health.worker.terminal.<launchRequestId>` events can wake `handleRemediationTelemetryEvent` before the next periodic tick. Startup prefers a `merge-agent` GitHub App installation token (`FOLLOW_UP_GH_AUTH_VIA_BROKER=true`, `FOLLOW_UP_GH_BROKER_ROLE=merge-agent`) for workspace preparation and push-back; ambient `gh auth token` is only a fail-safe fallback. Each tick first refreshes that runtime `GITHUB_TOKEN`/`GH_TOKEN`, then runs the reviewer-token refresh before reconciling or consuming work, so installation-token expiry and an unreadable operator Keychain item cannot strand the queue. The reviewer refresh re-resolves broker-enabled `GH_CLAUDE_REVIEWER_TOKEN`, `GH_CODEX_REVIEWER_TOKEN`, and `GH_GEMINI_REVIEWER_TOKEN` values before `consume` can spawn remediation workers that snapshot `process.env` and before `retry-comments` can post directly. For worker spawn handoff, the daemon passes the remediation-specific minimum remaining lifetime `ADVERSARIAL_REMEDIATION_WORKER_TOKEN_MIN_LIFETIME_MS` (existing code default 22m), so cached broker tokens that are still long enough for a reviewer subprocess but too short for remediation handoff are rejected before `consume`. Broker failures leave any prior runtime/reviewer token in place and retry on a bounded schedule. At startup, transient broker transport/HTTP failures use a short launchd retry (`FOLLOW_UP_GH_TRANSIENT_RETRY_SECONDS`, default 60s, clamped to 1-300s); permanent configuration/response-contract failures retain the one-hour crash-loop suppression sleep when ambient `gh` cannot seed a token. A separate stopped-job archive sweep also runs on its own cadence within the same daemon loop. Capacity defaults to one active remediation worker; set `ADVERSARIAL_REMEDIATION_MAX_CONCURRENT_JOBS=<n>` to allow bounded parallel workers across different PRs. The daemon clamps that env knob to `8` and logs when it had to clamp, so a typo cannot fan out an unbounded number of worker spawns in one tick. The daemon still refuses to spawn a second active remediation worker for the same `repo`/`prNumber`, using a case-insensitive repo/pr exclusion key because workers mutate the existing PR branch. If a SIGTERM arrives or one pending job fails during launch preparation, the daemon stops the current drain after the in-flight iteration and keeps filling any remaining safe capacity for unrelated PRs instead of aborting the whole tick. The tick loop itself runs in-process (no `node` subprocess fork on every tick), so macOS TCC trust is granted once for the daemon's `node` binary and reused for the daemon's lifetime — eliminates the per-tick "node would like to access data from other apps" prompts that an earlier subprocess-per-tick design produced. Per-spawn TCC popups when a fresh subprocess starts (a different problem, and one that fires on **both** the watcher's first-pass reviewer subprocess and the follow-up daemon's remediation worker — see `docs/MACOS-TCC.md` for which binaries each path execs and why FDA on those binaries is a security tradeoff worth reading before granting).
- Follow-up consumption also treats active AMA closer dispatch records under `data/follow-up-jobs/ama-closer-dispatches/` as same-PR reservations. `dispatching` records reserve only while their launch timestamps are valid and within the reclaim window; missing or malformed timestamps are treated as stale so corrupt records do not hold the PR forever. `dispatched` records reserve while the observed worker status is active or unknown, and release once the record reports a terminal status such as `succeeded`, `completed`, `failed`, `failed-without-merge`, `cancelled`, `superseded`, or `not-found`, or once the record ages past the same reclaim window the `dispatching` branch uses. That age escape exists because a worker that dies without ever writing a terminal ledger status would otherwise keep a non-terminal status forever and wedge its PR out of the consume loop (`deferredSamePR=N` logged alongside `activeAtStart=0` and full capacity). Age is measured from the record's most recent timestamp across `lastObservedAt`, `lastAttemptedAt`, `dispatchedAt`, and `createdAt` -- the latest of them, not the first one present. That distinction is load-bearing: `dispatchedAt` and `lastAttemptedAt` are stamped once at launch and never refreshed, while `lastObservedAt` is re-stamped on every tick that observes a live worker, so reading a launch stamp first would age out a closer that is still running. Unlike `dispatching`, a `dispatched` record with no parseable timestamp at all keeps its reservation, because a launch-only record must stay held until it is first observed. Malformed or concurrently removed dispatch files are skipped during the scan so one bad record does not blind later active reservations.
- Handoff wake marker files under `data/handoff-wake/` are a best-effort latency optimization, not the correctness path. The native writer path is allowed only when the caller UID matches the canonical owner of the existing wake directory, or the tool root before that directory exists. Cross-user callers must route marker creation through `sudo -A -H -u <owner>` so they do not create or re-parent daemon-owned coordination state under a worker UID. If owner resolution, sudo handoff, directory setup, or `fs.watch` setup fails, the signal is missed and the daemon falls back to its scheduled timer.
- Handoff telemetry logs under `data/handoff-events/` follow the same ownership rule as wake markers. A daemon may append natively only when its UID matches the canonical owner of the existing telemetry directory, the shared `data/` directory, or the tool root before either exists; cross-user callers append through `sudo -A -H -u <owner>` so daily JSONL files are not first-created under a worker UID that can lock out the canonical daemon.
- Before spawning a remediation worker, `consume` checks whether the PR touches `.github/workflows/*.yml` or `.yaml`. Embedded changed-file paths on the job are authoritative when present; otherwise the daemon fetches the live PR file list with `gh pr view` using the selected remediation push-token environment and bounded transient retries. If changed files cannot be proven after retry, the claimed job is moved back to `pending/` with `lastWorkflowPushPreflightFailure.recoverable=true` instead of failing open and spawning a worker that will later hit an unavoidable push rejection. For workflow-touching PRs, the daemon also probes the selected remediation push token's OAuth scopes or GitHub App workflow permission with bounded transient retries; exhausted transient probe failures requeue the job, while a proven missing workflow capability fails before spawn with `workflow-push-capability-missing` and a critical operator alert. When workflow-push merge-agent escalation is enabled, the broker token mint uses the same bounded transient retry schedule before classifying the escalation as unavailable, and the minted merge-agent token is applied only to an isolated preflight environment rather than the daemon's global `process.env`. Ambient `gh` credentials are repaired with `gh auth refresh -s workflow`; configured tokens such as `ADVERSARIAL_REMEDIATION_PUSH_TOKEN` require rotating that environment token or changing its underlying GitHub App installation permissions, because refreshing ambient `gh` auth does not modify the configured token.
- At remediation push time, raw git/gh commands treat GitHub authentication rejection as distinct from transient network failure. For ambient follow-up/remediation credentials, the worker forces one broker re-mint of `GITHUB_TOKEN`/`GH_TOKEN` through `FOLLOW_UP_GH_AUTH_VIA_BROKER` / `FOLLOW_UP_GH_BROKER_ROLE`, rebuilds the git credential-helper environment, and retries the rejected command once immediately. If no replacement ambient credential lands, the worker fails fast and reports that the work is committed locally but unpushed. If the selected push credential is configured/static (`ADVERSARIAL_REMEDIATION_PUSH_GITHUB_TOKEN`, `ADVERSARIAL_REMEDIATION_PUSH_TOKEN`, `REMEDIATION_PUSH_GITHUB_TOKEN`, or `REMEDIATION_PUSH_TOKEN`), the worker does not refresh unrelated ambient credentials or retry with the same token; rotate that selected token or update its broker-managed source, then requeue/rerun the remediation.
- When the follow-up daemon's broker refresh cannot prove every broker-enabled reviewer token is still safe for remediation-worker subprocess handoff, the tick continues through reconcile, heartbeat, stale-claim sweep, comment retry, and maintenance, but skips only `consume` for that tick so no worker snapshots a token below `ADVERSARIAL_REMEDIATION_WORKER_TOKEN_MIN_LIFETIME_MS`.
- The adversarial-watcher heartbeat path resolves in order from `ADVERSARIAL_WATCHER_HEARTBEAT_PATH`, then `HQ_ROOT/.adversarial-watcher/heartbeat.json`, then `data/watcher-heartbeat.json` under the tool root. When the stable `HQ_ROOT` heartbeat path is selected, the watcher asserts the caller UID matches the canonical owner of `HQ_ROOT` or the existing heartbeat directory before writing; cross-user callers fail the heartbeat write closed instead of creating wrong-owned liveness state under another operator's HQ root. If the canonical daemon finds an existing heartbeat file owned by another UID, it logs the degraded state, unlinks that file, rechecks ownership, and writes a fresh heartbeat so a prior rogue run cannot permanently wedge liveness.
- Before each reviewer spawn, the watcher may probe the local routing tier (LiteLLM) at `WATCHER_ROUTING_TIER_READINESS_URL` (default `http://127.0.0.1:4000/health/readiness`) with timeout `WATCHER_ROUTING_TIER_READINESS_TIMEOUT_MS` (default `2000`). Set `WATCHER_ROUTING_TIER_READINESS_PROBE_DISABLED=1` to bypass the gate. Probe failures do not consume `review_attempts`; ordinary routing failures settle through the transient `cascade` path, while HTTP 529/backend capacity signals settle as `provider-overloaded`. The watcher retries a failed probe with short bounded backoff before deferring a PR, caches successful probes for the rest of the current poll tick, and caches failed probes only briefly so later PRs in the same tick can re-probe after a short proxy bounce. At the start of every poll, the watcher also runs a durable reviewer-pass timeout sweep: `reviewer_passes.status='running'` rows with `ended_at IS NULL` and a parseable `started_at` older than `reviewer.running_pass_timeout_seconds` (default `3600`) are compare-and-swap updated by `pass_id` to `status='failed'` with `failureClass='reviewer-timeout'`, `failureReason='running-pass-timeout'`, and the threshold in metadata. When the matching `reviewed_prs` row is still `review_status='reviewing'`, the sweep also compares active claim evidence before releasing that durable claim to `pending-upstream`, increments `infra_auto_recover_attempts`, and records reviewer-timeout backoff evidence. A pass metadata `reviewerSessionUuid` must match when present; legacy pass rows without that metadata can still match by same head plus a pass `started_at` at or after the durable claim start. If the infra cap is already exhausted, the row is marked `failed` with timeout evidence for operator inspection. Rows that completed after selection, or whose active claim evidence no longer matches the stale pass, are left intact and are not counted as reaped. `npm run pipeline-health -- --json` and `--prometheus` surface active `provider-overloaded` backoffs and `quota-exhausted` holds under reviewer degradation.
...
- Public PR comments are **idempotent**. Each comment body embeds an HTML-comment marker keyed by `(jobId, round, action)` (prefix `adversarial-review-remediation-marker:`). Before posting, the poster runs a bounded `gh api` lookup on the PR's existing comments and skips the create if a previous attempt already landed on GitHub even though the local CLI saw a timeout. Generic follow-up lookup failures fall through to posting (best-effort: a duplicate is preferable to silent loss); HAM terminal-remediation audit comments are stricter and fail closed after bounded transient retries so the changed-head authority path cannot duplicate terminal audit evidence on a read outage.
...
- The watcher runs a stale-state reaper for offline-period recovery **both at startup and periodically from the poll loop** (`createStaleStateReaperTicker`, default spacing `ADVERSARIAL_STALE_STATE_REAPER_INTERVAL_MS=900000`). Startup-only scheduling was the CLR-02 defect: the reclaimer worked, but ran once per process lifetime, so leases orphaned by a dead closer sat until the next watcher restart (observed 2026-08-26: 11 of 11 non-terminal leases stale, every holder pid dead, 0 reclaimed). The tick is a no-op until its interval elapses, is never re-entered, and never throws, so it cannot delay or fail a poll. It marks aged `reviewer_passes.status='running'` rows abandoned, releases stale AMA closer leases, and unlinks syntactically corrupt/truncated AMA closer lease files as abandoned partial-failure state. Transient filesystem read errors on otherwise valid lease files are logged and left intact; the cursor advances so one persistent failure cannot blind later leases, then retries the failed name after a full lexical rotation. A lease that disappears after discovery is treated as a normal concurrent completion and does not halt the page. Closer-lease discovery scans the directory while retaining only bounded lexical candidate heaps, resumes after the last handled name, and admits at most `ADVERSARIAL_STALE_CLOSER_LEASE_ENTRY_SCAN_LIMIT` names (default `250`) and `ADVERSARIAL_STALE_CLOSER_LEASE_READ_LIMIT` JSON records (default `50`) per pass. Closer-lease reclamation is two-tier. The floor is `ADVERSARIAL_STALE_CLOSER_LEASE_MS` (default `21600000`, 6h) and applies to every lease whose holder cannot be proven dead. A lease is released at the shorter `ADVERSARIAL_DEAD_HOLDER_CLOSER_LEASE_MS` (default `1800000`, 30m — the same age the health surface pages at via `amaCloserLeaseMaxAgeMs`, and the same age `AMA_CLOSER_DISPATCHED_LEASE_RECLAIM_AGE_MS` already uses in the dispatch path) **only** when its `holderHost` is this host AND its recorded `watcherPid` is dead here. Leases written before CLR-02 have no `holderHost` and therefore always get the 6h floor; so do foreign-host leases, unusable pids, liveness probes that throw, and corrupt files. The floor is never bypassed on a guess, only on positive evidence that nothing holds the lease. Finished leases are retired: a lease that is `status='terminal'` with a resolved non-null `terminalOutcome` and older than `ADVERSARIAL_TERMINAL_CLOSER_LEASE_PRUNE_MS` (default `604800000`, 7d) is deleted, because the bounded scan was otherwise spent reading 50-day-old finished records instead of live ones (observed: 575 leases, 560 of them finished). Pruning never touches a non-terminal lease, a `terminal` lease whose `terminalOutcome` is still null (an unfinished reconciliation, not finished work), a corrupt record, or a record that cannot be aged; set the variable to `0` or `off` to disable it entirely. Release and prune decide from the same already-read page, so bounding the directory costs no extra I/O. The reaper writes `data/recovery-reaper/closer-lease-cursor.json` atomically after each page so read-capped leases remain next while persistent read or release failures are retried on wrap without starving the directory tail. This name-based continuation remains stable when owners concurrently delete earlier leases; names inserted before the cursor are picked up when the cursor wraps. When releasing a stale lease also needs to reset a transiently exhausted AMA closer budget, the reaper keeps the lease as the retry signal while continuing cursor progress. The sweep is fail-safe: errors are logged and must not prevent the watcher from polling.
...
- Bounding: `DEFAULT_MAX_REMEDIATION_ROUNDS = 2` in `src/follow-up-jobs.mjs` because `medium` is the default risk class; new jobs use risk-class caps of `low=1`, `medium=2`, `high=3`, and `critical=4` (was `3` before 2026-05-06 and `6` before 2026-05-02). The cap is enforced **PR-wide**, not per-job. The watcher reads the PR's prior remediation-round count from the durable follow-up-jobs ledger (`summarizePRRemediationLedger`) and seeds each new follow-up job's `currentRound` with that count. Fresh jobs normally re-derive the cap from the current risk class; if the latest PR ledger cap is higher than the current tier, the watcher carries that elevated cap forward so legacy in-flight PRs and operator-raised budgets do not silently lose budget mid-deploy. After the cap is consumed, the next review still runs and uses the lenient final-round verdict-categorization addendum embedded in `prompts/code-pr/reviewer.last.md`; the watcher surfaces that owed terminal pass as `owed-post-budget-final-review` so callers do not treat a moved head as another normal in-budget cycle. Completion is proved by a completed reviewer pass whose `started_at` is at or after the durable terminal timestamp where the remediation ledger reached its cap; reviewer counts are never compared with author-remediation counts, because a reviewer may coalesce several pushes. Later hammer-moved heads are suppressed once that proof exists. That addendum relaxes the *categorization* bar (style / nits / future-proofing concerns become non-blocking) but does **not** relax the automated merge gate — the verdict stays `Request changes` whenever any finding remains unless a current scoped `operator-approved` label deliberately overrides review/remediation state for that head. `requestReviewRereview` in `src/review-state.mjs` does **not** implement a cooldown — it refuses the reset only on hard guardrails (review row missing, malformed-title terminal, PR not open, already pending). The PR-wide round cap is the only re-arm bound. (An earlier doc claim about a per-PR rereview cooldown was inaccurate.)
- Stage-keyed prompts are part of the shipped loop contract. Reviewer runs select `prompts/code-pr/reviewer.first.md`, `reviewer.middle.md`, or `reviewer.last.md` via `pickReviewerStage(reviewAttemptNumber, completedRemediationRounds, maxRemediationRounds)`. Missing or invalid reviewer context falls back to the `first` prompt; `middle` is used only when the run is provably a re-review after at least one completed remediation round; `last` is used once the completed-round count reaches the stored cap. Every code-PR reviewer stage also carries the canonical doc-currency check: when a diff changes in-repo persistent-store shape, reviewers verify the matching `docs/data-model/NN-*.md` file plus `docs/data-model/catalog.json`; when a diff changes an in-repo module public/operational surface, reviewers verify the matching `modules/<name>/<name>-walkthrough.md`; submodule/superproject-only docs are not invented locally, but skipped obligations must remain auditable. This check intentionally remains blocking on `reviewer.last.md` because stale data-model and walkthrough docs are operator-facing contract drift; at cap exhaustion, a PR still blocked only on accepted doc-drift risk requires the same scoped `operator-approved` recovery path as any other explicitly accepted final-round finding. Remediation workers similarly select `prompts/code-pr/remediator.{first,middle,last}.md` via `pickRemediatorStage(remediationRound, maxRemediationRounds)`, with bad or missing round context falling back to `first` instead of silently framing the worker as mid-cycle. Every remediator stage treats those same data-model and module-walkthrough updates as in-scope remediation when the worker's own patch changes the corresponding surface, runs `node scripts/validate-data-model-catalog.mjs` when that script exists for in-repo data-model edits, treats validator failures as red checks, records a missing validator script as an operator follow-up rather than a red check by itself, and records skipped superproject-doc obligations in the machine-readable reply.
- Every remediator prompt stage tells the worker to refuse any dirty worktree (tracked or untracked) via `git status --porcelain --untracked-files=all`, fetch and rebase onto `origin/<baseBranch>` before code changes, then run a mandatory `git cherry origin/<baseBranch> HEAD` contamination audit before commit-and-push. The pre-edit base fetch is still mandatory, but GitHub smart-HTTP transport failures such as receive timeouts, connection resets, TLS/SSL failures, HTTP 5xx, `early EOF`, and `RPC failed` are retried up to three total attempts before the worker may stop with `operationalBlockers[]` titled `fetch-origin-<baseBranch>`; permanent auth, permission, repository-not-found, or missing-ref errors are not retried. Legacy jobs that predate `baseBranch` persistence are lazily hydrated from GitHub before spawn/reconcile; if the real PR base cannot be proven, the job fails instead of guessing `main`. That is a real head-rewrite, not a cosmetic sync step: the worker's eventual push updates the PR head SHA, and any head-scoped operator label event (`operator-approved`, `merge-agent-requested`) that was attached to the pre-rebase head is stale for the rebased head until an operator reapplies it. The watcher already evaluates those labels against the current head SHA, so a remediation rebase can legitimately make an earlier operator label stop authorizing merge-agent or override behavior on the next tick.
- Detached remediation workers force `GIT_EDITOR=true`, `GIT_SEQUENCE_EDITOR=true`, and `GIT_TERMINAL_PROMPT=0`. `git rebase --continue` keeps the prepared message without opening `vi` in a process with no terminal; authentication prompts fail promptly instead of occupying the single default remediation slot.
...
- Reviewer-bot tokens (`GH_CLAUDE_REVIEWER_TOKEN`, `GH_CODEX_REVIEWER_TOKEN`, `GH_GEMINI_REVIEWER_TOKEN`) are best-effort: a missing token at startup is logged as a warning, the daemon still runs consume/reconcile, and the comment poster records `token-env-missing` for later retry once the token is restored. Reconcile-time PR comments and first-pass review posts pass `--reviewer-login`, `--auth-mode env-token`, and the selected `GH_*_REVIEWER_TOKEN` env to the GitHub adapter so the write posts under the registered reviewer App/PAT identity, not the host's fallback Clio/ambient identity. A 1Password outage at boot does not block remediation.
...
Leave `ADVERSARIAL_REVIEW_DEFAULT_REVIEWER` unset for the normal reviewer routing. The base opposite-agent route is `[codex]` → Claude, `[claude-code]` → Codex, `[clio-agent]` → Claude because Clio dispatches Codex-family writers, and `[gemini]` / `[pi]` / `[opencode]` / `[hermes]` → Codex. The exported route helpers and watcher then apply `reviewer.gemini.mode` (default `off`) through the same effective-route helper, so the default keeps the base route and token mapping. Set `reviewer.gemini.mode=always-on` after Antigravity reviewer accounts are provisioned to route `[codex]`, `[claude-code]`, and `[clio-agent]` first-pass reviews to Gemini with `GH_GEMINI_REVIEWER_TOKEN`; `[gemini]`, `[pi]`, `[opencode]`, and `[hermes]` still keep Codex with `GH_CODEX_REVIEWER_TOKEN`. Set `reviewer.gemini.mode=fallback` to use Gemini only while the base reviewer is quota-capped. Gemini is never allowed to review `[gemini]` PRs; even an operator Gemini pin is stripped back to Codex for that builder class. Set `ADVERSARIAL_REVIEW_DEFAULT_REVIEWER` to `codex` to lock Claude out of first-pass reviews, to `claude` to force Claude reviews for every supported builder class, or to `gemini` to force the native Gemini reviewer runtime where the Gemini-on-Gemini hard guard permits it. The forced reviewer also overrides the Gemini mode layer and selects the matching reviewer bot token (`GH_CODEX_REVIEWER_TOKEN`, `GH_CLAUDE_REVIEWER_TOKEN`, or `GH_GEMINI_REVIEWER_TOKEN`). The watcher validates this knob at startup and exits non-zero on invalid values; under launchd that means a typo in this optional knob intentionally crash-loops the watcher until the env is fixed, with a `FATAL config: ADVERSARIAL_REVIEW_DEFAULT_REVIEWER...` banner in stderr identifying the cause. If the pin deliberately causes same-family review outside the Gemini hard guard, the watcher logs a waiver and the posted review body includes a cross-model-waiver note for auditability. That waiver is audit-only and does not alter the merge gate; a same-family `Comment only` review can still satisfy the automated gate unless an operator or branch-protection policy adds a separate human check.

The opt-in local OSS shadow path is enabled per PR by applying the `run-local-review-shadow` label and setting `ADVERSARIAL_REVIEW_LOCAL_SHADOW_MODEL` to a known local model family. This path is non-gating: the hosted review is posted and the follow-up handoff is queued independently, while the local shadow artifact remains a diagnostic under `data/local-review-shadow/`. The authoritative env knobs are `ADVERSARIAL_REVIEW_LOCAL_SHADOW_MODEL`, `ADVERSARIAL_REVIEW_LOCAL_SHADOW_BASE_URL`, `ADVERSARIAL_REVIEW_LOCAL_SHADOW_TIMEOUT_MS`, and `ADVERSARIAL_REVIEW_LOCAL_SHADOW_API_KEY` (falling back to `LITELLM_API_KEY` for auth). The LiteLLM base URL must be HTTP(S) loopback (`localhost`, `127.0.0.1`, or `::1`) before the PR diff or hosted review body is sent. Retryable failures such as timeouts, transport errors, 429s, and 5xx responses write a `warn-skip` state and can be retried later; permanent local configuration, auth, non-retryable 4xx client, invalid JSON, empty-response, and non-loopback URL failures write a terminal `skipped` state with a classified reason.

Leave `ADVERSARIAL_REVIEW_DEFAULT_REMEDIATOR` unset for the cross-model adversarial default (per-PR mapping: `[codex]`→claude-code, `[claude-code]`→codex, `[clio-agent]`→claude-code; missing `builderTag` falls through to codex). The shipped airlock watcher and follow-up LaunchAgent templates currently pin this knob to `claude-code`, making `claude-code` the effective fleet default until that template value is changed or explicitly overridden. Set it to `codex`, `claude-code`, or `gemini` to pin every PR's remediator globally — useful during a codex weekly-budget squeeze or when one model is temporarily unavailable. `gemini` (GMW-03) runs remediation on the native gemini CLI headless (`gemini --approval-mode yolo --skip-trust -m <best-model>`, prompt via stdin, scrubbed OAuth env), giving the fleet a third remediator when both the codex and claude lanes are capped; its commits carry the `gemini-remediation` provenance trailer. Unknown non-empty values fail closed instead of silently falling back to an expensive or unintended agent. The follow-up daemon validates this knob at startup before consuming work, and the claimed-job failure path also traps invalid values so a direct helper call cannot strand a job in `in-progress/`; because this is a global config error rather than a job-specific worker failure, the claimed job is restored to `pending/` with `lastConfigValidationFailure.code=config-validation-failure` so it can run after the env is corrected.
...
The old table (`low/medium=1`, `high/critical=3`) no longer applies to new jobs: medium now gets the auto-queued retry round, and critical gets one extra iteration before operator handoff. After the stored remediation cap is consumed, the next adversarial review still runs, but automatic stale-head re-review is suppressed after that post-budget final review completes. Standalone stale-head re-reviews are also capped by the same effective round budget so they cannot bypass the remediation-side cap. If a human accepts the current head, the `operator-approved` label can dispatch merge-agent, but only when the latest matching GitHub `labeled` event is attributable, appears after the PR timeline code event for the current head SHA, and was not applied by the PR author. It bypasses missing/unknown verdicts, `Request changes`, active/unknown remediation state, and claimable remediation rounds; not-mergeable state, failed or pending CI, closed PRs, and explicit skip labels remain hard gates. PRs without a discoverable spec linkage fall back to `medium` -> `2` rounds.
...
AMA also owns two operator labels that are initialized idempotently in every watched repository when the AMA label initializer runs. Existing labels are preserved by name: if an operator has customized the color or description, the initializer does not overwrite it. `adversarial-merge-blocked` is a current-head AMA hard stop. It refuses AMA closure regardless of review verdict, risk class, `operator-approved`, or `adversarial-merge-requested`; PR authors may apply it to block their own PR, and AMA never removes it automatically. `adversarial-merge-requested` is a current-head request for AMA closure on an otherwise risk-class-blocked PR. It bypasses only the AMA risk-class gate, requires an attributable non-author label event on the current head, and still requires `operator-approved` for the high/critical/unknown two-key path plus all structural gates: open, non-draft, mergeable, green CI, configured branch-protection gate, no remediation pending, no hard-stop labels, and `--match-head-commit`. Raw label presence is not authority for either label once event evidence is supplied: stale request evidence is ignored, stale/unapplied block evidence is the only way to ignore a still-attached block label, and missing or null block evidence fails closed on label presence.
...
`merge-agent-requested` is the stronger stuck-branch request. It is accepted only from an attributable GitHub `labeled` event scoped to the current head SHA, carrying durable audit identity (`eventId` / `nodeId` plus `observedAt`), and fresh relative to the latest PR update on that same head. Same-login application is valid at single-operator scale; freshness and head scoping are the safety checks. When scoped, it can bypass missing or unknown verdicts, mergeability, failed or pending checks, and remediation-round gates so merge-agent can clean the branch. It still does not bypass closed/merged PRs, active remediation, explicit skip labels, or duplicate-dispatch protection. Stale or unattributed requests return `skip-merge-agent-requested-stale`.

`reviewer-cycle-cap-reached` is the watcher-owned stop marker for repeated review/remediation cycles. The watcher counts successful reviewer verdicts that still carry standing structured blocking findings across distinct PR head SHAs within `review_cycle_window_hours` (default `24`); settled `Comment only` or `Approved` verdicts with `## Blocking issues` set to `- None.` do not accrue cycle-cap budget. Once the next attempt would exceed `review_cycle_cap` (default `5`), it posts one PR-scoped escalation comment, applies this label, records the row as `review_status='failed'`, and pauses automatic reviewer dispatch. Later pushes while the cap pause remains active do not get duplicate escalation comments.

The only supported cap-pause override labels are `operator-approved`, `merge-agent-requested`, and `paused-for-redesign`. Applying `operator-approved` or `merge-agent-requested` removes `reviewer-cycle-cap-reached`, resets the cycle counter, and restores the row to `posted` so the existing current-head merge-agent override lane can evaluate the PR. Applying `paused-for-redesign` removes the cap label and resets the counter, but leaves the row `failed` with a redesign-specific reason; that label is an intentional pause, not a request to resume normal review.

If a reviewer repeatedly exits with `reviewer-timeout` / `Command no output for ...`, treat that as reviewer-infrastructure drag, not a substantive review round. The watcher backs off and, after the configured timeout fallback threshold, can switch to the alternate reviewer model for a recovery attempt only when an operator explicitly sets `ADVERSARIAL_REVIEW_TIMEOUT_FALLBACK_MODEL=claude|codex` (default `off`). Timeout exhaustion is scoped to the head SHA that the timed-out reviewer attempted; a newly pushed head must earn its own reviewer attempts instead of inheriting the previous head's timeout count. When timeout exhaustion happens after a remediation worker already completed and requested re-review, the watcher routes the PR through the same AMA/merge-agent coexistence matrix used by the normal merge-agent handoff. With AMA disabled, the existing `reviewer-timeout-exhausted` merge-agent trigger runs unchanged. With AMA enabled, the watcher first tries the AMA closer; true AMA eligibility misses still park at await-operator unless a fresh scoped `merge-agent-requested` event authorizes the operator-fallback lane, but AMA launch/status failures recover by dispatching merge-agent with `AMA_OPERATOR_MERGE_AGENT_OVERRIDE=true` so a transient AMA outage does not become a durable manual park. Inspect `data/follow-up-jobs/merge-agent-dispatches/` for a dispatch or `data/follow-up-jobs/merge-agent-skips/` for an explicit blocker. `skip-blockers-present` / `skip-blocking-findings-unknown` on that trigger means the latest structured review still has standing blockers or an unknowable blocker state, so remediate or apply a scoped operator override before merging; `skip-not-mergeable` means the branch needs a rebase/conflict rescue before automation can continue; `skip-checks-*` means external CI/check state, not the missing reviewer, is the current blocker.
...
The `merge_agent.stuck_pre_spawn` pager is incident-scoped to `(repo, PR, LRQ)`. It pages when that LRQ first exceeds the 30-minute pre-spawn threshold, then suppresses unchanged observations for the configured debounce (six hours by default). A replacement LRQ for the same PR pages immediately; elapsed `stuckForMinutes` alone is a reminder, not a new severe event. Set `watcher.stuck_dispatch_alert_debounce_ms` only when an operator needs a different reminder cadence.
...
`merge-agent-skip`, `do-not-merge`, and `no-merge-hold` are unbypassable skip labels. They win over both override labels and surface as `skip-operator-skip`. `merge-agent-stuck` remains the terminal handoff marker for watcher-owned auto-retries and phantom-handoff escalation, but a scoped current-head `merge-agent-requested` label is the explicit operator recovery action for that state and can bypass `merge-agent-stuck` for one recovery dispatch. Stale or unattributed requests still fail closed as `skip-merge-agent-requested-stale`.
...
`merge-agent-dispatched` is the watcher-owned lifecycle marker for an active merge-agent dispatch. The watcher adds it after a successful `hq dispatch`; if the label add fails, the failed add is recorded under `data/follow-up-jobs/merge-agent-lifecycle-cleanups/` and retried on later ticks instead of being lost as a log-only side effect. That unresolved `transition: "dispatched-label-add"` cleanup record is part of the watcher-owned lifecycle contract: if the worker later dies in a terminal retryable state before the label ever lands, the watcher still treats the dispatch as "died without handoff" and may reclaim it instead of wedging forever on `skip-already-dispatched`. Once the label is cleared and the recorded LRQ is terminal-failed, the watcher stamps `phantomHandoffObservedAt` into the per-head dispatch record on the first tick that proves the handoff gap and starts the 60-minute grace from that durable observation time rather than from the original dispatch start. That detection is proactive: each watcher tick also inspects current-head dispatch records whose public `merge-agent-dispatched` marker is already gone, so a phantom handoff is still observed even when the PR never re-enters the normal dispatch revisit path. If the gap persists through the grace window, the watcher first records durable pending phantom-handoff comment state on the dispatch record, then converges the `merge-agent-stuck` label and owed operator comment from that ledger on later ticks. That ledger-first ordering means a partial failure after the label transition cannot silently lose the human-facing explanation; later ticks keep retrying the missing label and/or missing comment idempotently until the handoff is visible. On merge or close, the watcher writes a durable cleanup record before the PR leaves the open-set query, attempts `hq dispatch cancel <lrq>`, removes any per-PR no-progress lane ledger under `data/watcher-no-progress-lane/`, and retries that cleanup on later ticks until both worker cancellation and label removal converge. On merge, the watcher also writes a durable DAG owed-work record before marking the PR merged, then runs `hq dag autowalk-on-merge --repo <repo> --pr <n>` through the paced retry worker at `data/follow-up-jobs/dag-autowalk-on-merge/`; the record is removed only on command success and otherwise keeps stdout/stderr plus exit details for later ticks or operator repair. On merge or close the watcher also queues the owed operator-surface (Linear) triage sync under `data/follow-up-jobs/pending-triage-sync/` **before** it writes the terminal `reviewed_prs` mark, and drains it on later ticks via `retryPendingTriageSyncs`. That ordering is load-bearing (TREC-01): the terminal mark used to sit behind the remote Linear call inside one try block, so a Linear failure threw past the mark and the row stayed `pr_state=open` as the retry vehicle — and because `review:queue_starvation` and `review:terminal_but_unmerged` both select on `pr_state=open` and then threshold on elapsed age, a held-open row produced an alert whose age only grew and which could never clear. The durable record is now the retry vehicle, so the terminal fact is recorded the moment GitHub confirms it while the Linear obligation still converges. Malformed triage-sync records are logged and renamed to `.failed` beside the original file so they remain inspectable but no longer consume the active `.json` drain budget. Every lifecycle sweep writes an attestation to `data/pr-lifecycle-reconcile/state.json` naming the PRs it could not resolve against GitHub; `review:pr_lifecycle_mirror_unverified` reads it, and the two age-based findings stamp `mirrorVerified` into their payloads from it so an operator can tell a phantom entry from real backlog without hand-checking each PR. Operator `npm run merge-agent:cancel` now writes the same cleanup record when it gets a retryable partial failure on an open PR, so a stranded `merge-agent-dispatched` label is replayed by the watcher instead of being a best-effort one-shot. Non-terminal cancel failures keep the label present as the public retry signal; terminal cancel outcomes such as "already terminated" still proceed to label removal so stale labels do not linger forever. Cleanup retries are paced and capped per poll so an HQ or GitHub outage does not monopolize the watcher; malformed cleanup records are logged loudly instead of being silently dropped.

`merge-agent-recovery-in-flight` is a transitional handoff marker the merge-agent applies when it dispatches a failure-recovery worker (MAR-C, paired with agent-os MAR-B/C). The merge-agent owns the WRITE path: it applies the label BEFORE removing `merge-agent-dispatched` so the PR always carries one of the two in-flight markers during the handoff. The recovery worker owns the REMOVE path on every terminal outcome (success, fail, cancel). The watcher owns the READ path: in both `reconcilePhantomHandoffEscalation` (per-job) and `reconcileProactivePhantomHandoffs` (per-tick scan), the presence of this label suppresses the 60-minute phantom-handoff grace timer the same way `merge-agent-dispatched` does. Without this signal the 60-min stuck escalation fires the moment the merge-agent removes `merge-agent-dispatched` and exits — even when a legitimate recovery worker is alive and running. The 2026-05-31 incident on PRs #1186/#1188/#1194 (false `merge-agent-stuck` 60 min after recovery handoff) is exactly the gap this label closes. The label's max age is bounded by `_PHANTOM_HANDOFF_RECOVERY_IN_FLIGHT_MAX_MINUTES` (currently 120 min — 2× the grace window): if the label persists longer than that, the recovery worker has almost certainly died before its cleanup path could run, the watcher treats the marker as stale, logs `merge_agent.recovery_in_flight_label_max_age_exceeded`, and falls through to the existing grace/escalation path so a stuck label cannot freeze the PR forever. The ceiling clock is the LABEL's `LabeledEvent.createdAt` from the PR's GitHub timeline (fetched via `fetchLatestLabelEvent`), NOT the original merge-agent dispatch time — keying the ceiling off the dispatch time would fire the 120-min limit instantly on any merge-agent that ran longer than 2h before dispatching recovery. If the watcher cannot resolve the label-add timestamp (gh API failure, no matching timeline event), it logs `merge_agent.recovery_in_flight_label_age_unresolved` and fails closed into the grace path — the safer direction is false-positive on stuck, not silently wedging the PR behind an unresolvable label. Stale-label recovery: operators can clear a stuck `merge-agent-recovery-in-flight` label by hand; the watcher will then either dispatch a fresh merge-agent attempt (if other gates allow) or surface the underlying state via the normal phantom-handoff path.

### Comment-only to merge example

Use this path when checking whether the docs still match live watcher behavior:

1. Review round 2 posts `Comment only` for current head `abc123`.
2. The watcher evaluates merge-agent dispatch for `abc123`; this is not an inline merge branch in `src/follow-up-jobs.mjs`.
3. If the latest structured `## Blocking issues` section is `- None.`, the watcher dispatches merge-agent and writes the handoff to `data/follow-up-jobs/merge-agent-dispatches/<repo>-pr-<n>-abc123.json`.
4. If a `merge-agent-requested` label was applied on older head `def456`, it is stale for `abc123` and does not authorize the new head. Current-head scoping is load-bearing.
5. If `abc123` already has a dispatch record or still has a live `merge-agent-dispatched` handoff, the watcher returns `skip-already-dispatched` instead of launching a duplicate worker.
6. If the same `Comment only` review still carries a real blocking finding in `## Blocking issues`, the watcher returns `skip-blockers-present` (ARP-06 / #157) and parks the PR until a fresh clean review, scoped `operator-approved`, or scoped `merge-agent-requested` override exists for the current head.
...
When the watcher refuses a re-review because the current PR head has failed
external CI and requeueing the latest follow-up job consumes the stored round
cap, the resulting `stopped:max-rounds-reached` job carries
`remediationPlan.stop.ciRegression=true`. Pipeline health uses that
machine-readable marker, not free-form stop prose, to treat the pending
...
- `retrigger-review --exact-head-now` is the audited operator recovery surface for "review this exact head now." It keeps the same exit-code contract, but adds two guarded recoveries that previously required multiple manual surfaces: if the latest active follow-up job is provably stale for an older head (`job.revisionRef != reviewed_prs.revision_ref`), the command stops that stale job before re-arming review; and if the watcher row is still `reviewing`, the command still refuses by default, but can either cancel the reviewer and wait for it to exit (`--cancel-active-review`) or, when the operator has already verified the process is gone, force-reset the stale `reviewing` row (`--allow-active-review-reset`). Both paths append the same operator mutation ledger row and preserve the explicit operator re-review marker that lets the next watcher tick spend one intentional hard-review-ceiling bypass on the current head.
- `retrigger-remediation` requeues the latest terminal follow-up job and optionally bumps `remediationPlan.maxRounds`. It starts with a remediation worker responding to the last posted review; it does not reset `reviews.db` for another review first. If the watcher row is already `review_status='pending'`, reviewer dispatch is still deferred while the latest follow-up job is `pending` or `inProgress`, so the worker cannot race a fresh review verdict. It only accepts terminal jobs in `failed`, `completed` with `reReview.requested=true`, or `stopped:{max-rounds-reached,round-budget-exhausted,daemon-bounce-safety,review-settled,no-progress,stale-review-head,revision-superseded,stale-heartbeat}`. `stopped:review-settled` is accepted because the automatic loop stops on Comment-only reviews, while an explicit operator flag means "address the remaining non-blocking findings." The requeue records that one-shot operator override on `remediationPlan.nextAction={type:'consume-pending-round', operatorOverride:true, requestedAt, requestedBy, operatorVisibility:'explicit'}`; the next `claimNextFollowUpJob` must honor it by skipping the `review-settled` early-stop once, then consume it by rewriting `nextAction` to `worker-spawn`. While the requeued job is `pending` or `inProgress`, the adversarial gate remains pending even if the stored review body is still Comment-only. It still refuses `stopped:operator-stop` and `stopped:rereview-blocked` so explicit operator halts and watcher refusals do not silently spawn another worker. If the worker fails or is stopped before leaving a durable `reReview.requested=true` reply, no fresh adversarial review is queued; apply `retrigger-review` separately when an operator wants a review without another worker pass. If a launch crash happens after claim but before spawn, let reconcile mark the in-progress job `failed` (or use the documented operator stop/reset path), then apply `retrigger-remediation` again; active jobs intentionally refuse duplicate retriggers. Its exit-code contract is also stable: `0=success`, `1=blocked`, `2=usage`, `3=reason input`, `4=runtime`.
- `diagnose-stuck-rereview --apply` is an automation watchdog, not an operator retrigger surface. It scans pending rows with aged `rereview_requested_at`, then preserves that original FIFO/latency timestamp and the existing `rereview_reason` marker byte-for-byte while writing a `stuck-rereview-watchdog` watcher wake for the affected repo/PR/head. It does not clear `review_attempts`, `last_attempted_at`, `failed_at`, `failure_message`, `reviewer_head_sha`, or retry counters, and rows carrying `failed_at` stay visible in read-only diagnostics while the apply path skips them for terminal-failure retry/finalization. Re-arms are tracked in `data/follow-up-jobs/stuck-rereview-watchdog.json` by `(repo, pr, head)`, use exponential backoff from the stuck threshold, record the original request time, prune entries whose last successful apply is older than seven days whenever the state file is rewritten, and stop after three attempts; cap exhaustion is reported as a skipped row rather than an apply failure so a capped row cannot consume the window forever. If the row eligibility check succeeds but writing the wake fails, the same state entry/backoff is still recorded so a persistent wake-file failure cannot bypass the cap. Its `--apply` exit-code contract is `0=apply pass completed (including skipped/capped/benign-race rows)` and `5=at least one non-benign apply failure`; the read-only contract (`0`/`4`) is unchanged, and rows carrying `failed_at` stay visible to read-only exit-4 alerting.
- The watchdog reads the locally cached `reviewed_prs.pr_state` as a candidate pre-filter only. It deliberately does not make the merged/closed call — that decision is owned by the watcher's claim guard, which derives it authoritative-live before spawning. A locally stale `open` row therefore costs at most the per-`(repo, pr, head)` cap in watcher wakes and never mutates the review row.
- The follow-up job buckets are listed once per `diagnose-stuck-rereview` invocation and indexed by the exact candidate `(repo, pr)` filename prefixes, not re-scanned per candidate row. `main()` is synchronous and runs inside the daemon tick, so the directory cost must stay independent of candidate count; the `completed` bucket grows monotonically.
- To disarm the daemon's `stuck-rereview-apply` tick step during an incident, set `ADVERSARIAL_STUCK_REREVIEW_APPLY_ENABLED=0` (also accepts `false`/`no`/`off`) in the follow-up daemon's environment and bounce the daemon. The tick then logs `stuck-rereview-apply skipped disarmed via ...` and takes no watchdog action; unset or empty keeps it armed, and the read-only `diagnose-stuck-rereview` diagnostic is unaffected either way.
...
Fresh transient reviewer failures (`cascade`, PR-local `oauth-broken`, `provider-overloaded`, `reviewer-timeout`, `launchctl-bootstrap`, and `daemon-bounce`) settle directly to `review_status='pending-upstream'`, increment `infra_auto_recover_attempts` immediately, and wait for reviewer-lane/routing-tier recovery rather than first becoming `failed`. This is the documented path for those classes; they intentionally bypass the `failed` SQL compare-and-swap claim. Repeated `oauth-broken` failures across distinct PRs also open a model-scoped outage file under `data/reviewer-credential-outages/`; the watcher preflight holds further spawns for that model until the file's `nextProbeAt`, then records a fresh probe reservation and lets one normal reviewer attempt through as a recovery probe. Once the model-wide outage is active, another OAuth failure records outage-transient evidence without incrementing `infra_auto_recover_attempts` and preserves the reserved retry window when it is still in the future. Success clears the outage and re-arms only open, lease-free `pending` / `pending-upstream` rows parked with the outage-transient marker; in-flight `reviewing` rows and terminal `failed` evidence are left unchanged.

For `review_status='failed'`, the watcher may auto-retry only when the row matches the bounded infrastructure-recovery classifier for legacy or explicitly terminal failed evidence. The row must be rediscovered as a current open PR in an active repo, watcher drain must be inactive, active follow-up/backoff/merge-agent handoff gates must allow the spawn, memory admission must pass, routing-tier readiness must be healthy, and the dedicated infrastructure SQL compare-and-swap must promote the row to `reviewing`. The prior `failed_at` and `failure_message` remain intact until that claim succeeds. Infrastructure-class failed rows (`cascade`, `provider-overloaded`, `reviewer-timeout`, `launchctl-bootstrap`, reviewer-spawn `oauth-broken` including untagged GitHub credential-rejection messages such as `gh: Bad credentials (HTTP 401)`, `quota-exhausted`, and `reviewer-command-failed` rows whose stored evidence starts with `[unknown] Command failed` or `[unknown] Command failed with code <n>`, including stdout/stderr tails) use that dedicated claim to atomically increment `infra_auto_recover_attempts` only if the row is still the same infrastructure class. The same dedicated claim also accepts lease-released `pending` rows that still carry the exact `failed_at` and `reviewer_head_sha` evidence the watcher just classified, so infra retries consume the infra budget instead of falling through to the generic pending claim. Once that counter reaches the cap, failed rows remain `failed` for operator inspection, while lease-released pending rows are finalized to `failed` without incrementing attempts so they stop polling as pending work while preserving the evidence. `quota-exhausted` is special: before any recovery claim, the watcher holds the row until `quota_reset_at_utc`, a parsed reset from `failure_message`, or the fixed fallback window has cleared, and that hold does not consume `infra_auto_recover_attempts`. Before retrying `reviewer-command-failed`, the watcher uses the persisted reviewer session/start evidence to query GitHub for a matching reviewer-bot review; an already-posted review is reconciled by marking the row `posted`, while missing evidence or a failed review probe leaves the row `failed` rather than risking a duplicate review. The counter resets after a successful posted review or an intentional re-review re-arm. `forbidden-fallback`, unknown/non-infrastructure failures, inactive repos, PRs missing from GitHub discovery, closed or merged PRs, and rows skipped during watcher drain are not silently reset.

For `review_status='failed-orphan'`, inspect the GitHub PR first if the bounded auto-reclaim pass does not clear it. Auto-reclaim may reset the row only after the stored reviewer lease expires, the infrastructure recovery cap has room, no late reviewer post is found, and the local process evidence proves the original reviewer is gone: the reviewer PGID no longer exists, or the PGID exists but its command line does not contain the stored `reviewer_session_uuid` and is therefore a recycled process group. If a late reviewer post is found during auto-reclaim, or if reviewer reattach finds a matching posted review on the current head while the matching reviewer PGID is still alive, the watcher sends `SIGKILL` to the confirmed matching process group before marking the row `posted`, so the same failed-orphan row does not burn GitHub API quota on every tick and cannot immediately become a duplicate-review spawn window. For the live-PGID posted-review case, the watcher performs the bounded cleanup recheck after the kill attempt and writes `data/reviewer-cleanup-findings/<session>.json` only if the confirmed matching process group remains alive, logging `reviewer_posted_process_group_cleanup_finding` as the cleanup signal and rechecking the finding on later ticks until the process exits or the PGID no longer matches the original reviewer session. Rows that already became sticky before this reconciliation existed can be backfilled with `npm run reconcile-posted-orphans -- --apply --limit <n>`; the command scans open `failed-orphan` rows, accepts matching reviewer-bot reviews on the stored head, uses `last_attempted_at` when the reviewer start timestamp is missing or corrupt, refuses rows where neither timestamp is parseable instead of omitting the lower time bound, links a previously unlinked reviewer pass artifact, and queues recovered follow-up work only for that newly linked artifact when no same-revision follow-up job already exists. If no orphaned reviewer post landed, run `npm run retrigger-review -- --repo <slug> --pr <n> --reason "<verified reason>"`; that is the supported manual clear path and lets the next watcher tick publish the next non-failure `agent-os/adversarial-gate` state. Do not hand-edit `data/reviews.db` or the local gate-record file to clear it. During reviewer reattach, transient GitHub head/review probe failures such as timeouts, rate limits, and 5xx responses leave the row in `reviewing` for the next watcher tick instead of creating a sticky `failed-orphan`; non-transient probe failures still fail closed because safe recovery cannot be proven. When null-PGID recovery has no historical reviewer start timestamp, the GitHub review lookup omits the lower time bound rather than using the current time, so operators should not assume a recovered review had to be newer than the watcher restart.
...
- **Default ON (this deployment + code default):** After the cap is consumed and the verdict is still `Request changes`, the watcher first checks the latest structured `## Blocking issues` section. If the review is legacy/malformed and lacks the structured blocking section, it parks with `skip-blocking-findings-unknown` because blocker state is not knowable. When blocker state is known-zero, the merge-agent is automatically dispatched with the `final-pass-on-budget-exhausted` trigger plus `MERGE_AGENT_BLOCKING_FINDING_COUNT=0`. On that zero-blocker path, the merge-agent's `comment_only_followups.py` sub-worker **applies every actionable in-scope reviewer finding inline** — trivial polish and substantive non-trivial work alike. Light-to-medium fixes are pushed, checked, and merged without another review. Only major in-PR refactors trigger a fresh adversarial review pass. When blocker state is known-nonzero, the watcher permits exactly one automatic blocker-remediation dispatch per PR/head progression with the distinct trigger `final-pass-blocker-remediation`, `MERGE_AGENT_BLOCKING_FINDING_COUNT=<n>`, and `MERGE_AGENT_BLOCKER_REMEDIATION_REQUIRED=1`; that mode is the machine-readable no-merge discriminator for downstream code and adapters. The merge-agent applies the blocking findings inline, pushes the remediated head, and requests a fresh adversarial review (`reReview.requested=true` / `awaiting-rereview`) instead of merging its own blocker fixes.

  If the posted review head is stale after the review cycle is already exhausted, the watcher may launch the HAM terminal-remediation lane against the current PR head instead of requesting a fresh review first. The dispatch context must preserve the posted review head as `reviewedSha` and pass the live head separately as `targetRemediationSha`. The AMA dispatch record under `data/follow-up-jobs/ama-closer-dispatches/` is keyed by its dispatch-record `headSha`; for current target-redrive records that is the live remediation target, while `reviewedSha` remains the stale reviewed-commit audit field. The exhausted-lane marker lives in `dispatchReason: "exhausted-final-hammer"`, not in any SHA field. A clean stale head is not re-hammered by default; it can resume HAM only when the watcher proves the live head is a closer/HAM-authored commit, which covers partial failures where HAM pushed a repair but exited before merging.
  ...
  If the fresh post-remediation review still reports blockers, the watcher parks the PR with `skip-blockers-present`, records `handoffRequired=true`, and applies `merge-agent-stuck` so the terminal operator handoff is visible in the normal stuck queue. Recover by fixing the blockers and getting a fresh structured review, by applying scoped `operator-approved` to accept the risk, or by applying scoped `merge-agent-requested` when you intentionally want the merge-agent escape hatch for that head.
...
When the loop hits the cap, the first thing to check is which mode the watcher is in: `launchctl print gui/$(id -u <watcher-owner>)/ai.laceyenterprises.adversarial-watcher.<watcher-owner> | grep MERGE_AGENT_FINAL_PASS_ON_REQUEST_CHANGES` (NOT the follow-up daemon — that env was the pre-2026-05-16 contract and is no longer the control point). If unset or `=1` the flag is ON, inspect the merge-agent decision. `dispatch` with the `final-pass-on-budget-exhausted` trigger means blocker state was known-zero and the merge-agent may merge after bounded inline follow-up fixes. `dispatch` with `final-pass-blocker-remediation` means the single no-merge blocker pass is in flight and downstream code should also see `MERGE_AGENT_BLOCKER_REMEDIATION_REQUIRED=1`; a successful pass must push fixes and request a fresh adversarial review rather than merge. `skip-blockers-present` or `skip-blocking-findings-unknown` means the PR is intentionally parked; inspect `data/follow-up-jobs/merge-agent-skips/<repo>-pr-<n>-<headSha>.json` before assuming the watcher missed it. A `handoffRequired=true` skip also applies `merge-agent-stuck`, so the PR should surface in the operator stuck queue without log spelunking. If `suggestions_unable_to_apply` is non-empty, verify that corresponding Linear tickets were filed and that the merge-agent did not park the current PR merely because follow-up work exists. If `blockers_observed` fired, inspect `.adversarial-follow-up/followups-reply.json` on the merge-worker workspace (NOT the receipt) for the redacted payload and decide whether to add `do-not-merge` / `merge-agent-skip` or to split the blocker into its own PR.
