...
- If applying a finding would require more than one PR (multi-PR scope, schema-migration plan, refactor that changes PR intent), the sub-worker records it under `suggestions_unable_to_apply` with a concrete reasoning string. If the sub-worker also made and force-pushed code/config edits, the merge-agent routes that to `awaiting-rereview` so the next reviewer pass evaluates both the new head and the punt. If no edit was made, the merge-agent hard-refuses / operator-handoffs because there is no durable same-head rereview trigger.
