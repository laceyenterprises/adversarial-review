...
| `review:closer_head_rereview_deadlock` | a closer-authored head is still open after the merge attempt resolved without closing, while a posted review on the superseded head still blocks verdict settlement | ticket | the watcher re-arms a review for the current closer head, `retrigger-review` explicitly re-arms it, or the PR merges/closes |
