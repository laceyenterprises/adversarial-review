...
- Watcher startup runs a stale-state reaper for offline-period recovery. It marks aged `reviewer_passes.status='running'` rows abandoned, releases stale AMA closer leases, immediately releases younger leases when the recorded watcher PID is provably dead, and unlinks syntactically corrupt/truncated AMA closer lease files as abandoned partial-failure state. Transient filesystem read errors on otherwise valid lease files are logged and skipped so the next sweep can retry instead of deleting active duplicate-dispatch protection. Closer-lease discovery is bounded per pass by `ADVERSARIAL_STALE_CLOSER_LEASE_ENTRY_SCAN_LIMIT` (default `250`) and `ADVERSARIAL_STALE_CLOSER_LEASE_READ_LIMIT` (default `50`); each pass consumes at most the entry budget from the directory iterator, sorts only those admitted names, and writes `data/recovery-reaper/closer-lease-cursor.json` atomically after the last lease actually read so read-capped JSON leases are retried instead of skipped. When releasing a stale lease also needs to reset a transiently exhausted AMA closer budget, the reaper keeps the lease if that reset cannot be persisted; the retained lease is the retry signal for the next sweep. The sweep is fail-safe: errors are logged and must not prevent the watcher from polling.
