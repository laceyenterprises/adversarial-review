...
`reviewer-cycle-cap-reached` is the watcher-owned stop marker for repeated review/remediation cycles. The watcher counts successful reviewer verdicts across distinct PR head SHAs within `review_cycle_window_hours` (default `24`); once the next attempt would exceed `review_cycle_cap` (default `5`), it posts one PR-scoped escalation comment, applies this label, records the row as `review_status='failed'`, and pauses automatic reviewer dispatch. Later pushes while the cap pause remains active do not get duplicate escalation comments.
