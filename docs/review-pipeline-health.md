...
- `review_pipeline_conflicting_open_prs_collected`: 1 when the GitHub open-PR
  listing for the conflicting-PR diagnostic was collected and every attempted
  local merge-tree probe completed without blind spots, 0 when the snapshot is
  blind. Configured cap/deadline truncation is reported on the snapshot as
  `truncated: true` plus `unprobedPrs`; it is not by itself a blind collector
  state. A blind snapshot is not equivalent to zero conflicts.
...
conflicting-open-PR evidence instead of partial coverage. Rows with transient
`mergeable=UNKNOWN` are counted as blind evidence, never as a clean zero.
It initializes a throwaway git repo with `origin` set to the target repository,
resolves each base ref through `git ls-remote`, fetches any missing head/base
...
repo and the checkout whose object store is reused read-only through
`GIT_ALTERNATE_OBJECT_DIRECTORIES`; the collector never fetches into that
checkout. `ADVERSARIAL_REVIEW_PIPELINE_HEALTH_CONFLICTING_PR_MAX_PROBED_PRS`
...
5000) bound attempted probes inside the 300s production job budget; PRs skipped
only because those bounds were reached are recorded as unprobed truncation, not
as unreadable blind evidence.
...
- Keychain-mode Claude reviewer runtime probes use the same unprivileged
  ...
  probe reports `claude-broker-transport-no-launchctl`.
...
| `review:conflicting_open_prs_unreadable` | SEN-02 `blind`: the GitHub open-PR listing was unavailable or truncated, GitHub returned transient `mergeable=UNKNOWN`, one or more attempted per-PR merge-tree probes could not be collected, or GitHub reported `CONFLICTING` while the fresh-base local merge-tree was clean. Configured cap/deadline truncation records `truncated: true` and `unprobedPrs` but does not make the snapshot blind by itself. The snapshot cannot distinguish "zero conflicts" from "not measured" when this finding is active. Never a health verdict. | ticket | the GitHub listing is non-truncated, mergeability is known, and every attempted per-PR probe completes without blind spots |
...
- `ADVERSARIAL_REVIEW_PIPELINE_HEALTH_AFH_FALLBACK_SUPERMAJORITY_MIN_SELECTIONS`
  (default `5`)
- `ADVERSARIAL_REVIEW_PIPELINE_HEALTH_AFH_FALLBACK_SUPERMAJORITY_DISTINCT_PR_FLOOR`
  (default `2`)
