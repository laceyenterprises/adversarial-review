...
- Before making code changes, rebase the PR branch onto a freshly-fetched \`origin/${baseBranch}\` so the remediation lands on top of current trunk. Use **exactly** this sequence — improvised variants will silently re-introduce already-merged commits as duplicates and corrupt the PR diff for the next reviewer pass:
  ...
  3. Rebase onto the freshly-fetched base ref only (NOT local \`${baseBranch}\`, and NOT the remote PR branch): \`git rebase origin/${baseBranch}\`. Git's built-in cherry-pick detection drops commits whose patch matches upstream; do not pass any flag that disables it. Never rebase onto \`origin/<this-pr-branch>\` to "catch up" with another writer; that folds moving PR-branch history into your remediation workspace.
  ...
  6. If the remote PR branch changed while you were working (i.e., \`git fetch origin <this-pr-branch>\` then \`git merge-base --is-ancestor origin/<this-pr-branch> HEAD\` returns non-zero), rebase your local commits onto the new PR head — do NOT bail to a human just because the head moved. The other writer is almost always another remediation worker or the operator pushing a fix; replaying our commits on top of theirs is the cheap, correct recovery, and \`--force-with-lease\` at push time catches any further race. Exact sequence:
     a. \`git fetch origin <this-pr-branch>\` (refresh the remote-tracking ref).
     b. \`git rebase origin/<this-pr-branch>\`. Git's built-in cherry-pick detection drops commits whose patch matches what the other writer already applied, so no double-fix. This IS the "rebase onto the PR branch" case — step 3's prohibition was about base-catchup, not moving-head recovery.
     c. Re-run the step 5 branch-contamination audit (\`git cherry origin/${baseBranch} HEAD\`) — the rebase may have folded in commits already on base. If contamination appears, stop with the \`branch-contamination\` operationalBlocker as in step 5.
     d. Re-run the relevant tests on the rebased head.
     e. Push with \`git push --force-with-lease=<this-pr-branch>:origin/<this-pr-branch>\` so a further concurrent push aborts your push rather than overwriting it.
     f. ONLY if the rebase in step (b) produces conflicts you cannot resolve cleanly (the other writer changed the same hunks you did), \`git rebase --abort\` and stop with an \`operationalBlockers[]\` entry titled \`stale-pr-head\`. \`stale-pr-head\` is reserved for actual rebase-conflict cases; do not use it for a clean head-moved-and-rebased recovery. Cite the conflicting paths in the blocker entry so the operator knows where the parallel-write collision happened.
  7. After the rebase(s) succeed and the audit passes, re-run the relevant tests so the rebase outcome is validated alongside the original fix.
