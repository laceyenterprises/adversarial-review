...
- Watcher startup runs a stale-state reaper for offline-period recovery. It marks aged `reviewer_passes.status='running'` rows abandoned, releases stale AMA closer leases, immediately releases younger leases when the recorded watcher PID is provably dead, and unlinks corrupt/truncated AMA closer lease files as abandoned partial-failure state. The sweep is fail-safe: errors are logged and must not prevent the watcher from polling.
