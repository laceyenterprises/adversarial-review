...
const REVIEWER_ROUTE_BY_MODEL = {
  ...
  gemini: { reviewerModel: 'gemini', botTokenEnv: 'GH_GEMINI_REVIEWER_TOKEN' },
};
...
test('CFG-02 resolveDefaultReviewer accepts gemini as an explicit reviewer pin', () => {
  const tmp = makeTmp();
  try {
    const topPath = join(tmp, 'top.yaml');
    writeYaml(topPath, 'version: 1\nroles:\n  reviewer: gemini\n');
    ...
    assert.deepEqual(route, REVIEWER_ROUTE_BY_MODEL.gemini);
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
});

test('CFG-02 resolveDefaultReviewer rejects unsupported MHX-09 title tags as config reviewer pins', () => {
  const tmp = makeTmp();
  try {
    const topPath = join(tmp, 'top.yaml');
    writeYaml(topPath, 'version: 1\nroles:\n  reviewer: opencode\n');
