...
- If the remote PR branch moves while the worker is running, the worker must stop with `operationalBlockers[]: [{ title: "stale-pr-head", ... }]` and let the orchestrator/operator replay the patch on the fresh PR head. Auto-rebasing onto `origin/<this-pr-branch>` is intentionally unsupported because branch rewrites, squashes, and force-push cleanups can make that replay resurrect commits another writer removed.
