...
| `review:operational_blocker_stopped_round` | at least one stopped remediation round recorded `operationalBlockers`; auth blockers use canonical `missing-auth` / `auth-failure` titles and may include a bounded rescue bundle under `$HQ_ROOT/remediation-rescue/` | ticket | the stopped round is requeued, rescued/resumed, or aged out of the stopped queue/archive population; rescue bundles are best-effort retained for 7 days and capped at 256 MiB total |
