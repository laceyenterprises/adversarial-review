...
- Before making code changes, rebase the PR branch onto a freshly-fetched \`origin/${baseBranch}\` so the remediation lands on top of current trunk. Use **exactly** this sequence — improvised variants will silently re-introduce already-merged commits as duplicates and corrupt the PR diff for the next reviewer pass:
  ...
  6. If the remote PR branch changed while you were working, do not rebase onto it. Stop with an \`operationalBlockers[]\` entry titled \`stale-pr-head\` so the orchestrator/operator can replay your patch on the fresh head. Record the current remote head SHA and the local HEAD SHA in the worker log so the next human can verify the divergence. A moved PR head is distinct from the base-branch contamination audit in step 5: even if \`git cherry origin/${baseBranch} HEAD\` is clean, a rewritten or force-pushed PR branch can still make automatic replay unsafe.
  7. After the rebase succeeds and the audit passes, re-run the relevant tests so the rebase outcome is validated alongside the original fix.
