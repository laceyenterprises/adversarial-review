...
- Before making code changes, rebase the PR branch onto a freshly-fetched \`origin/${baseBranch}\` so the remediation lands on top of current trunk. Use **exactly** this sequence — improvised variants will silently re-introduce already-merged commits as duplicates and corrupt the PR diff for the next reviewer pass:
  ...
  2. Force-fetch first (never rebase against a cached remote-tracking ref): \`git fetch --prune origin ${baseBranch}\`. The fetch must succeed before rebasing. Treat GitHub smart-HTTP transport failures such as \`Recv failure: Operation timed out\`, \`Operation timed out\`, connection resets, TLS/SSL failures, HTTP 5xx, \`early EOF\`, \`RPC failed\`, or \`remote end hung up unexpectedly\` as transient: retry the same exact fetch up to three total attempts with short backoff before stopping. Do not retry permanent auth, permission, repository-not-found, or missing-ref errors. If the bounded attempts are exhausted, surface an \`operationalBlockers[]\` entry titled \`fetch-origin-${baseBranch}\` with the attempt count and final stderr, and do not rebase.
