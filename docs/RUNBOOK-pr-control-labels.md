...
| `duplicate-family-hold` | PR-wide while present | Watcher-owned projection from `data/reviews.db` duplicate-family rows. | Blocks AMA, hammer routing, merge-agent dispatch, and fast-merge while the candidate belongs to an unresolved live unsuppressed duplicate family. The watcher removes the hold after the family deactivates or suppression/override evidence releases that candidate. |
