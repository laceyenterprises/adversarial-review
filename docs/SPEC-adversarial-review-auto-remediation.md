...
- `diagnose-stuck-rereview --apply` is a daemon watchdog for rows that look abandoned after a durable worker rereview request. It is not an operator override: it must use a non-operator reason prefix, must not clear reviewer failure evidence or retry counters, and must not satisfy the explicit `retrigger-review:` bypass predicate. It must preserve `rereview_requested_at` as the original FIFO and latency clock, may refresh only `rereview_reason` on an open pending row without `failed_at`, and must request a watcher wake for the affected repo/PR/head as the retry nudge. Repeated refreshes are capped and backed off per `(repo, pr, head)`, state entries older than seven days are pruned on write, and cap exhaustion is surfaced as a skipped row rather than an apply failure so capped rows cannot consume the apply window forever.
