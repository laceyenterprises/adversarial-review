...
- Local shadow reviews are an opt-in, **non-gating** reviewer adjunct. Add the PR label `run-local-review-shadow` and set `ADVERSARIAL_REVIEW_LOCAL_SHADOW_MODEL=<provider/model>` on the reviewer host to request one. Supported model-family metadata is maintained in `src/reviewer.mjs`; if the configured local model cannot be proven distinct from the PR builder family and hosted reviewer family, the request is recorded as `hosted-only` and no LiteLLM call is made. `ADVERSARIAL_REVIEW_LOCAL_SHADOW_TIMEOUT_MS` controls the LiteLLM execution timeout and defaults to five minutes. The hosted reviewer writes durable request state, including the diff and prompt context, before posting the GitHub review at `data/local-review-shadow/requests/<requestKey>.json`, marks that request `ready` only after the hosted review post succeeds, and stores the hosted review text for the later local prompt. The follow-up daemon runs a bounded `local-shadow-reconcile` sweep every tick after follow-up job reconciliation and before remediation consume; `ADVERSARIAL_REVIEW_LOCAL_SHADOW_SWEEP_LIMIT` controls the per-tick request cap and defaults to 5. If the reviewer process crashes after the hosted post but before writing the shadow marker, the sweep can recover `hostedReview.posted` from the durable review-state row and captured hosted-review body for the same PR head. The sweep writes markdown to `data/local-review-shadow/artifacts/<requestKey>.md` and transitions the request through `running`, `completed`, `hosted-only`, or `retryable-warning`; retryable warnings carry a backoff window controlled by `ADVERSARIAL_REVIEW_LOCAL_SHADOW_RETRY_BACKOFF_MS` (default ten minutes). Shadow artifacts are informational only: they do not change the hosted adversarial verdict, do not create or suppress follow-up remediation jobs, and do not affect `agent-os/adversarial-gate`. The LiteLLM subprocess receives only the allowlisted local routing environment (`PATH`, LiteLLM endpoint/config variables, and local model endpoint variables such as `OLLAMA_HOST`), not reviewer bot tokens, OAuth fallback API keys, broker credentials, 1Password variables, or the full watcher/reviewer process environment; large prompts are written to a local temp prompt file and passed by path instead of argv.
