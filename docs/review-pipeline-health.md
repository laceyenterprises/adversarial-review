...
| `review:reviewer_model_silent` | a reviewer model with a genuine posted review inside the 7d activity lookback has not posted again for 24h | ticket | the model posts another review inside the silence threshold, or no longer has posted-review activity inside the lookback |
