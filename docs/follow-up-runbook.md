...
- Before each reviewer spawn, the watcher may probe the local routing tier (LiteLLM) at `WATCHER_ROUTING_TIER_READINESS_URL` (default `http://127.0.0.1:4000/health/readiness`) with timeout `WATCHER_ROUTING_TIER_READINESS_TIMEOUT_MS` (default `2000`). Set `WATCHER_ROUTING_TIER_READINESS_PROBE_DISABLED=1` to bypass the gate. Probe failures do not consume `review_attempts`; they settle through the existing transient `cascade` path. The watcher retries a failed probe with short bounded backoff before deferring a PR, caches successful probes for the rest of the current poll tick, and caches failed probes only briefly so later PRs in the same tick can re-probe after a short proxy bounce.
