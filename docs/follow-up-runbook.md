...
- Watcher startup runs a stale-state reaper for offline-period recovery. It marks aged `reviewer_passes.status='running'` rows abandoned, releases stale AMA closer leases, immediately releases younger leases when the recorded watcher PID is provably dead, and unlinks syntactically corrupt/truncated AMA closer lease files as abandoned partial-failure state. Closer-lease discovery reads and deterministically sorts the complete directory name set, while `ADVERSARIAL_STALE_CLOSER_LEASE_READ_LIMIT` (default `50`) bounds file reads and cursor-based processing per pass. Concurrently deleted lease files are skipped as expected completion races; other transient filesystem read errors are logged and pause cursor advancement so the next sweep retries them instead of deleting active duplicate-dispatch protection. The reaper writes `data/recovery-reaper/closer-lease-cursor.json` with the canonical atomic-write helper after the last lease actually attempted so read-capped JSON leases are not skipped. When releasing a stale lease also needs to reset a transiently exhausted AMA closer budget, the reaper keeps the lease if that reset cannot be persisted; the retained lease is the retry signal for the next sweep. The sweep is fail-safe: errors are logged and must not prevent the watcher from polling.
