...
- If every changed file is non-executable text — a document (`.md`/`.txt`/`.rst`), a spec, a plan, a prompt, a fixture, or config comments — this is a DOCUMENTATION review, not a code review. A document cannot by itself cause a runtime crash, data loss, or a deadlock, so do NOT invent runtime failure modes for it. Evaluate only: internal consistency, whether claims are supported, contradiction of a decision recorded elsewhere, and safety/privacy of anything it instructs.
- When you claim a document says something wrong, quote the exact sentence you dispute. If the document explicitly states the choice you are about to flag (e.g. it says a file is deliberately `0600` per-owner, or "do NOT add a sleep here"), that is a deliberate decision, not a defect — do not file it.
