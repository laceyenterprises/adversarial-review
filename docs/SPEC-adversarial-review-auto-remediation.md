...
Each poll processes up to `DEFAULT_FML_MERGE_AGENT_PER_POLL_CAP = 5` rows unless operators override the cap with `FML_MERGE_AGENT_PER_POLL_CAP`. For each row the daemon must:
