...
- `diagnose-stuck-rereview --apply` is an automation watchdog, not an operator retrigger surface. It scans pending rows with aged `rereview_requested_at`, then refreshes only `rereview_requested_at` and `rereview_reason` with the non-operator `stuck-rereview-watchdog:` prefix. It does not clear `review_attempts`, `last_attempted_at`, `failed_at`, `failure_message`, `reviewer_head_sha`, or retry counters, and rows carrying `failed_at` stay on the terminal-failure retry/finalization path. Re-arms are tracked in `data/follow-up-jobs/stuck-rereview-watchdog.json` by `(repo, pr, head)`, use exponential backoff from the stuck threshold, and stop after three attempts; cap exhaustion returns exit code 5 so the existing daemon alert surface names the stuck row instead of writing forever.
