...
> **One-time macOS step on a fresh machine.** After the LaunchAgents are installed, approve **three** binaries in **System Settings → Privacy & Security → Full Disk Access**: `/opt/homebrew/bin/node`, `/opt/homebrew/bin/claude`, and the real Mach-O `codex` binary buried under the npm sub-package. The codex binary path is **operator-specific** — it lives under the active operator's home (`$HOME/.local/share/fnm/...` for the user the daemon runs as), and the architecture-specific vendor triple differs between Apple Silicon and Intel. Resolve the exact path the daemon will exec with the dynamic resolver in `docs/MACOS-TCC.md` rather than reusing a path from another machine or the shipped `.placey` example — approving the wrong binary leaves the popup loop in place. Without this approval, every remediation worker spawn fires a TCC popup ("node would like to access data from other apps"). Use `⌘⇧G` in the file picker to navigate into `/opt` and `~/.local` (Finder hides them). Full details and re-approval triggers: `docs/MACOS-TCC.md`.
