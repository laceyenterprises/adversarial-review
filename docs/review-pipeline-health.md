...
| `review:remediation_zero_throughput` | oldest claimable pending remediation job is older than the queue-starvation threshold (default 10m) while `follow-up-jobs/in-progress` is empty; pending jobs whose `remediationPlan.retryAfter` is still in the future are excluded as intentionally delayed quota/retry holds | ticket | a remediation job enters progress, the claimable pending queue drains, the oldest claimable pending job falls below threshold, or a retry-delayed job reaches its `retryAfter` and becomes claimable |
...
| `review:config_signature_drift` | a long-lived daemon's loaded config signature differs from disk for more than 10m | ticket | the daemon reloads the changed config successfully, the disk config is restored to the loaded signature, or the status file reports the daemon back in sync |
