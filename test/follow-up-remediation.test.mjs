...
const MERGE_AGENT_BROKER_ENV_KEYS = [
  'ADVERSARIAL_REMEDIATION_WORKFLOW_PUSH_ESCALATE_TO_MERGE_AGENT',
...
async function withRemediationRouteEnv(workDir, envOverrides, run) {
  const hqRoot = path.join(workDir, 'agent-os-hq');
  mkdirSync(hqRoot, { recursive: true });
...
test('workflow push capability detection retries transient gh API failures', async () => {
  let calls = 0;
  const logs = [];
  const capability = await inspectRemediationPushTokenCapability({
    env: { GITHUB_TOKEN: 'gho_without_workflow' },
    retryDelaysMs: [0, 0],
    ...
    execFileImpl: async (command, args) => {
      ...
      calls += 1;
      if (calls < 3) {
        const err = new Error('EIO: i/o error during TLS handshake');
        err.code = 'EIO';
        throw err;
      }
      return { stdout: 'HTTP/2 200\nx-oauth-scopes: repo, workflow\n', stderr: '' };
  ...
  assert.equal(capability.hasWorkflowCapability, true);
  assert.match(logs.join('\n'), /oauth-scope header probe transient failure/);
});

test('workflow changed-file lookup retries transient errors and fails closed when unavailable', async () => {
  let calls = 0;
  await assert.rejects(
    ...
    (err) => {
      ...
      assert.equal(err.code, 'workflow-push-changed-files-unavailable');
      return true;
    },
  );
  assert.equal(calls, 3);
...
test('workflow push preflight escalates workflow-file remediation to merge-agent broker token', async () => {
  const env = {
  ...
  const result = await assertWorkflowPushCapabilityForJob({
    ...
    env,
    retryDelaysMs: [],
    log: { log: (line) => logs.push(line), warn: (line) => logs.push(line) },
    readFileImpl: (filePath) => {
      assert.equal(filePath, '/tmp/broker-secret');
...
test('workflow push preflight fails closed when merge-agent broker mint fails', async () => {
  ...
  await assert.rejects(
    ...
    (err) => {
      assert.equal(err.name, 'WorkflowPushCapabilityError');
      assert.equal(err.isWorkflowPushCapabilityError, true);
      return true;
    },
  );
...
test('workflow push preflight honors disabled merge-agent escalation flag', async () => {
  ...
  await assert.rejects(
    assertWorkflowPushCapabilityForJob({
      ...
      fetchImpl: async () => {
        fetchCalls += 1;
        throw new Error('broker should not be called when escalation is disabled');
      },
      execFileImpl: async () => ({ stdout: 'HTTP/2 200\nx-oauth-scopes: repo\n', stderr: '' }),
      log: { log() {}, warn() {}, error() {} },
    }),
    /requires push token workflow capability/,
  );
  assert.equal(fetchCalls, 0);
});

test('workflow push preflight leaves non-workflow remediation unchanged and skips broker escalation', async () => {
  let fetchCalls = 0;
  let execCalls = 0;
  const result = await assertWorkflowPushCapabilityForJob({
    job: {
    ...
    fetchImpl: async () => {
      fetchCalls += 1;
      throw new Error('broker should not be called for non-workflow remediation');
    },
    execFileImpl: async () => {
      execCalls += 1;
      throw new Error('capability should not be inspected for non-workflow remediation');
    },
  });
  assert.equal(result.workflowTouch.touches, false);
  assert.equal(result.capability.source, 'ambient-gh');
  assert.equal(fetchCalls, 0);
...
test('workflow changed-file lookup preflight outage requeues instead of spawning or terminalizing', async () => {
  ...
  const result = await withOAuthTestEnv(rootDir, () => consumeFollowUpJobsUntilCapacity({
    ...
    execFileImpl: async (command, args) => {
      ...
      throw new Error(`unexpected command after workflow changed-file preflight outage: ${command} ${args.join(' ')}`);
    },
    spawnImpl: (...args) => {
  ...
  assert.equal(result.results[0].reason, 'workflow-push-changed-files-unavailable');
...
test('workflow-touching remediation without workflow push capability blocks before spawn, alerts, and marks needs-operator', async () => {
  ...
  const originalEscalationFlag = process.env.ADVERSARIAL_REMEDIATION_WORKFLOW_PUSH_ESCALATE_TO_MERGE_AGENT;
  process.env.ADVERSARIAL_REMEDIATION_WORKFLOW_PUSH_ESCALATE_TO_MERGE_AGENT = 'false';
  try {
    const result = await withOAuthTestEnv(rootDir, () => consumeFollowUpJobsUntilCapacity({
      rootDir,
      maxConcurrent: 1,
    ...
    if (originalEscalationFlag === undefined) delete process.env.ADVERSARIAL_REMEDIATION_WORKFLOW_PUSH_ESCALATE_TO_MERGE_AGENT;
    else process.env.ADVERSARIAL_REMEDIATION_WORKFLOW_PUSH_ESCALATE_TO_MERGE_AGENT = originalEscalationFlag;
  }
});

test('workflow push capability alert gives configured-token recovery instead of ambient gh refresh', async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  const created = createPendingRemediationJob(rootDir);
  writeFollowUpJob(created.jobPath, {
    ...created.job,
  ...
  const logs = [];
  const original = {
    ADVERSARIAL_REMEDIATION_PUSH_TOKEN: process.env.ADVERSARIAL_REMEDIATION_PUSH_TOKEN,
  ...
  process.env.ADVERSARIAL_REMEDIATION_PUSH_TOKEN = 'configured_without_workflow';
  process.env.ADVERSARIAL_REMEDIATION_PUSH_TOKEN_IDENTITY = 'remediation-app-installation';
  process.env.ADVERSARIAL_REMEDIATION_WORKFLOW_PUSH_ESCALATE_TO_MERGE_AGENT = 'false';
  try {
    const result = await withOAuthTestEnv(rootDir, () => consumeFollowUpJobsUntilCapacity({
      rootDir,
      ...
      execFileImpl: async (command, args, options) => {
        if (command === 'gh' && args[0] === 'api' && args[1] === '/' && args.includes('--include')) {
          assert.equal(options.env.GITHUB_TOKEN, 'configured_without_workflow');
          assert.equal(options.env.GH_TOKEN, 'configured_without_workflow');
          return {
            stdout: 'HTTP/2 200\nx-oauth-scopes: repo, read:org\n',
            stderr: '',
          };
        }
        throw new Error(`unexpected command after configured-token workflow preflight block: ${command} ${args.join(' ')}`);
      },
      spawnImpl: (...args) => {
        spawnCalls.push(args);
        return { pid: 9999, unref() {} };
    ...
    assert.equal(spawnCalls.length, 0);
    assert.equal(alerts.length, 1);
    assert.match(alerts[0].text, /Update ADVERSARIAL_REMEDIATION_PUSH_TOKEN to a PAT\/OAuth token with workflow scope/);
    assert.doesNotMatch(alerts[0].text, /gh auth refresh/);
    assert.match(alerts[0].structured.operatorAction, /Update ADVERSARIAL_REMEDIATION_PUSH_TOKEN/);
    assert.doesNotMatch(alerts[0].structured.operatorAction, /gh auth refresh/);

    const failedDir = getFollowUpJobDir(rootDir, 'failed');
    const failedFiles = readdirSync(failedDir).filter((name) => name.endsWith('.json'));
    assert.equal(failedFiles.length, 1);
    const failedJob = JSON.parse(readFileSync(path.join(failedDir, failedFiles[0]), 'utf8'));
    assert.match(failedJob.failure.workflowPushCapability.operatorAction, /Update ADVERSARIAL_REMEDIATION_PUSH_TOKEN/);
    assert.doesNotMatch(failedJob.failure.workflowPushCapability.operatorAction, /gh auth refresh/);
  } finally {
...
test('workflow push preflight escalates workflow-file remediations to merge-agent broker token', async () => {
  const env = {
    OAUTH_BROKER_URL: 'http://broker.test',
    OAUTH_BROKER_SHARED_SECRET_FILE: '/secret',
  ...
  const result = await assertWorkflowPushCapabilityForJob({
    ...
    fetchImpl: async (url, options = {}) => {
      brokerCalls += 1;
      assert.equal(url, 'http://broker.test/token?provider=github-app-merge-agent');
      ...
      return {
        ...
        async json() {
          return {
            ...
            access_token: 'ghs_merge_agent_workflow',
            metadata: {},
            expires_at: '2026-08-16T20:00:00.000Z',
          };
        },
      };
    },
    execFileImpl: async (command, args) => {
      assert.equal(command, 'gh');
      if (args[0] === 'api' && args[1] === '/' && args.includes('--include')) {
        return { stdout: 'HTTP/2 200\nx-oauth-scopes: repo\n', stderr: '' };
      }
      throw new Error(`unexpected gh probe: ${args.join(' ')}`);
    },
    log: {
      log: (line) => logs.push(line),
      warn: (line) => logs.push(line),
...
test('workflow push preflight fails closed when merge-agent broker mint fails', async () => {
  ...
  await assert.rejects(
    ...
    (err) => err.name === 'WorkflowPushCapabilityError',
  );
  assert.equal(brokerCalls, 1);
  assert.equal(env.ADVERSARIAL_REMEDIATION_PUSH_GITHUB_TOKEN, undefined);
});

test('workflow push preflight flag disables merge-agent escalation', async () => {
  const env = {
    ADVERSARIAL_REMEDIATION_WORKFLOW_PUSH_ESCALATE_TO_MERGE_AGENT: 'false',
    OAUTH_BROKER_URL: 'http://broker.test',
    OAUTH_BROKER_SHARED_SECRET_FILE: '/secret',
...
test('workflow push preflight leaves non-workflow remediations unchanged without broker mint', async () => {
  let brokerCalls = 0;
  const result = await assertWorkflowPushCapabilityForJob({
    job: {
      repo: 'laceyenterprises/finch',
  ...
  assert.equal(result.gated, false);
  assert.equal(result.workflowTouch.touches, false);
  assert.equal(result.escalated, undefined);
  assert.equal(brokerCalls, 0);
});
...
test('applyMergeAgentBrokerEnv resolves the per-harness provider by default (never a fixed merge-agent provider)', () => {
  for (const [workerClass, provider] of [
    ...
    assert.equal(evidence.requiresWorkflowPush, false);
...
test('applyMergeAgentBrokerEnv routes workflow-file worker pushes through merge-agent provider', () => {
  const env = {};
  const evidence = applyMergeAgentBrokerEnv(env, {
    MERGE_AGENT_AUTH_VIA_BROKER: 'true',
...
test('applyMergeAgentBrokerEnv routes workflow-file remediation pushes through merge-agent', () => {
  const env = {};
  const evidence = applyMergeAgentBrokerEnv(env, {
    OAUTH_BROKER_MERGE_AGENT_EXPECTED_APP_ID: '3978009',
    OAUTH_BROKER_MERGE_AGENT_EXPECTED_INSTALLATION_ID: '138360282',
  ...
  assert.equal(evidence.requiresWorkflowPush, true);
  assert.equal(evidence.harnessClass, 'codex');
  assert.equal(evidence.harnessIdentityHonored, false);
  assert.equal(evidence.fellBack, false);
});
...
test('remediationWorkerPushProvider selects merge-agent when workflow push is required', () => {
  const resolved = remediationWorkerPushProvider('codex', {}, { requiresWorkflowPush: true });
  assert.equal(resolved.provider, 'github-app-merge-agent');
  assert.equal(resolved.source, 'workflow-push-merge-agent');
  assert.equal(resolved.requiresWorkflowPush, true);
  assert.equal(resolved.harnessClass, 'codex');
});
...
function makeQueuedJob(rootDir, overrides = {}) {
  ...
  const changedFiles = Object.prototype.hasOwnProperty.call(overrides, 'changedFiles')
...
test('consumeNextFollowUpJob invokes oss-readiness --apply once, commits the mechanical fix, and records evidence', async () => {
  ...
  try {
    const created = createFollowUpJob({
      ...
      reviewPostedAt: '2026-06-20T08:00:00.000Z',
      critical: true,
    });
        ...
        execFileImpl: async (command, args, options = {}) => {
          ...
          if (command === 'gh' && args[0] === 'api' && /\/pulls\//.test(args[1])) {
            return {
              stdout: JSON.stringify({
                base: { ref: 'main' },
                head: { ref: 'codex/psh-03', repo: { full_name: 'laceyenterprises/clio' } },
              }),
              stderr: '',
            };
          }
          ...
          if (command === 'git' && args[2] === 'commit') {
            committed = true;
            capturedCommitEnv = options.env;
            return { stdout: '[codex/psh-03 abc123] Apply oss-readiness remediation\n', stderr: '' };
          }
...
test('consumeNextFollowUpJob surfaces unfixable oss-readiness baseline bumps without spawning', async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  const scriptPath = path.join(rootDir, 'fixtures', 'audit-oss-readiness-hardcodes.py');
  mkdirSync(path.dirname(scriptPath), { recursive: true });
  writeFileSync(scriptPath, '#!/usr/bin/env python3\n', 'utf8');
  ...
  try {
    const created = createFollowUpJob({
      ...
      reviewBody: '## Summary\noss-readiness-audit failed and requires a baseline bump.\n\n## Verdict\nRequest changes',
      reviewPostedAt: '2026-06-20T08:10:00.000Z',
      critical: true,
    });
          ...
          execFileImpl: async (command, args) => {
            ...
            if (command === 'gh' && args[0] === 'api' && /\/pulls\//.test(args[1])) {
              return {
                stdout: JSON.stringify({
                  base: { ref: 'main' },
                  head: { ref: 'codex/psh-03-baseline', repo: { full_name: 'laceyenterprises/clio' } },
                }),
                stderr: '',
              };
            }
  ...
  try {
    ...
    await withOAuthTestEnv(rootDir, async () => {
      ...
      const result = await consumeNextFollowUpJob({
        ...
        execFileImpl: async (command, args) => {
          ...
          if (command === 'gh' && args[0] === 'api' && /\/pulls\//.test(args[1])) {
            return {
              stdout: JSON.stringify({
                base: { ref: 'main' },
                head: { ref: 'codex/normal-fix', repo: { full_name: 'laceyenterprises/clio' } },
              }),
              stderr: '',
            };
          }
        ...
        spawnImpl: () => ({ pid: 7676, unref() {} }),
      });

      assert.equal(result.consumed, true);
      assert.equal(applyCalls, 0);
      assert.equal(result.job.remediationWorker.ossReadinessApply, null);
    });
  } finally {
    if (previousScript === undefined) delete process.env[OSS_READINESS_APPLY_SCRIPT_ENV];
    else process.env[OSS_READINESS_APPLY_SCRIPT_ENV] = previousScript;
...
test('health.worker.terminal succeeded topic waits for durable reply before treating the worker as exited', async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  const { claimed } = makeQueuedJob(rootDir, { prNumber: 730, linearTicketId: 'LAC-2730' });
  const hqRoot = path.join(rootDir, 'hq');
  const workspaceDir = path.join(hqRoot, 'adversarial-review', 'follow-up-workspaces', claimed.job.jobId);
  const promptPath = path.join(workspaceDir, '.adversarial-follow-up', 'prompt.md');
  mkdirSync(path.dirname(promptPath), { recursive: true });
  ...
  const { replyPath } = prepareCanonicalReply(rootDir, claimed.job, {
    hqRoot,
    launchRequestId: 'lrq_apc05_topic_success_unflushed',
  });
  ...
  const spawned = markFollowUpJobSpawned({
    ...
    worker: {
      ...
      completionShape: 'branch-push',
      launchRequestId: 'lrq_apc05_topic_success_unflushed',
      dispatchId: 'dispatch_apc05_topic_success_unflushed',
      hqRoot,
      workspaceDir,
      promptPath,
      replyPath,
  ...
  const result = await withHqRootEnv(hqRoot, async () => handleRemediationTelemetryEvent({
    rootDir,
    topic: 'health.worker.terminal.lrq_apc05_topic_success_unflushed',
    event: {
      lrq: 'lrq_apc05_topic_success_unflushed',
      status: 'succeeded',
    },
    now: () => '2026-04-21T10:25:00.000Z',
    execFileImpl: async (command, args) => {
  ...
  assert.equal(statusPolls, 1, 'missing reply must fall back to canonical HQ status polling');
  assert.equal(result.action, 'active');
  assert.equal(result.reason, 'hq-dispatch-running');
...
test('health.worker.terminal topic event ignores jobs moved after lookup', async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  const { claimed } = makeQueuedJob(rootDir, { prNumber: 732 });
  const hqRoot = path.join(rootDir, 'hq');
  const promptPath = path.join(rootDir, 'data', 'follow-up-jobs', 'workspaces', claimed.job.jobId, '.adversarial-follow-up', 'prompt.md');
  mkdirSync(path.dirname(promptPath), { recursive: true });
  writeFileSync(promptPath, 'prompt\n', 'utf8');
  ...
  const spawned = markFollowUpJobSpawned({
    ...
    worker: {
      ...
      dispatchMode: 'hq',
      completionShape: 'branch-push',
      launchRequestId: 'lrq_apc05_moved_before_read',
      dispatchId: 'dispatch_apc05_moved_before_read',
      hqRoot,
      workspaceDir: path.join(hqRoot, 'adversarial-review', 'follow-up-workspaces', claimed.job.jobId),
      promptPath,
      replyPath: path.join(hqRoot, 'dispatch', 'remediation-replies', claimed.job.jobId, 'remediation-reply.json'),
  ...
  const result = await handleRemediationTelemetryEvent({
    rootDir,
    topic: 'health.worker.terminal.lrq_apc05_moved_before_read',
    event: { status: 'succeeded' },
    now: () => {
      nowCalls += 1;
      if (nowCalls === 1) {
        rmSync(spawned.jobPath, { force: true });
      }
      return '2026-04-21T10:25:00.000Z';
    },
    execFileImpl: async () => {
      throw new Error('telemetry handler should not poll after the job record moved');
    },
    log: { warn() {}, error() {}, log() {} },
  });

  assert.equal(result.action, 'ignored');
  assert.equal(result.reason, 'launch-request-not-in-progress');
  assert.equal(existsSync(`${spawned.jobPath}.reconcile.lock`), false);
});

test('health.worker.terminal topic event reclaims corrupt reconcile claim locks', async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  const { claimed } = makeQueuedJob(rootDir, { prNumber: 731 });
  const hqRoot = path.join(rootDir, 'hq');
  const workspaceDir = path.join(hqRoot, 'adversarial-review', 'follow-up-workspaces', claimed.job.jobId);
  const promptPath = path.join(workspaceDir, '.adversarial-follow-up', 'prompt.md');
  mkdirSync(path.dirname(promptPath), { recursive: true });
  ...
  const { replyPath } = prepareCanonicalReply(rootDir, claimed.job, {
    hqRoot,
    launchRequestId: 'lrq_apc05_corrupt_claim',
  });
  writeValidReply(replyPath, claimed.job, {
    reReview: { requested: true, reason: 'Recovered despite a corrupt reconcile claim.' },
  });
  ...
  const spawned = markFollowUpJobSpawned({
    ...
    worker: {
      ...
      completionShape: 'branch-push',
      launchRequestId: 'lrq_apc05_corrupt_claim',
      dispatchId: 'dispatch_apc05_corrupt_claim',
      hqRoot,
      workspaceDir,
      promptPath,
      replyPath,
  ...
  writeFileSync(`${spawned.jobPath}.reconcile.lock`, '{', 'utf8');

  const result = await withHqRootEnv(hqRoot, async () => handleRemediationTelemetryEvent({
    rootDir,
    topic: 'health.worker.terminal.lrq_apc05_corrupt_claim',
    event: {
      lrq: 'lrq_apc05_corrupt_claim',
      status: 'succeeded',
    },
    now: () => '2026-04-21T10:25:00.000Z',
    execFileImpl: async (command, args) => {
...
test('tryAcquireFollowUpReconcileClaim waits one hour before age-based reclaim', () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  const inProgressDir = getFollowUpJobDir(rootDir, 'inProgress');
  mkdirSync(inProgressDir, { recursive: true });
  ...
  const early = tryAcquireFollowUpReconcileClaim({
    jobPath,
  ...
  assert.equal(early.ownerToken, 'old-live-pid-token');

  const claim = tryAcquireFollowUpReconcileClaim({
    jobPath,
    now: () => '2026-04-21T11:01:00.000Z',
    ownerPid: process.pid,
  });
...
test('consumeNextFollowUpJob routes agentos remediation through HQ branch-push dispatch without legacy flag', async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  ...
  await withOAuthTestEnv(rootDir, async () => {
    createFollowUpJob({
      ...
      reviewerModel: 'claude',
      linearTicketId: 'LAC-276',
      reviewBody: '## Summary\nRoute this through agentos orchestration.\n\n## Verdict\nRequest changes',
      reviewPostedAt: '2026-04-21T08:00:00.000Z',
      critical: false,
    });
    ...
    await withAppContractDispatchServer(async ({ requests }) => withRemediationRouteEnv(rootDir, {
      ...
      const result = await consumeNextFollowUpJob({
        ...
        execFileImpl: async (command, args) => {
          ...
          if (command === 'gh' && args[0] === 'api' && /\/pulls\//.test(args[1])) {
            return {
              stdout: JSON.stringify({
                base: { ref: 'main' },
                head: { ref: 'codex/fix-pr-76', repo: { full_name: 'laceyenterprises/clio' } },
              }),
              stderr: '',
            };
          }
          if (command === 'hq' && args[0] === 'dispatch' && args[1] === 'status') {
            return {
              stdout: JSON.stringify({
                status: 'queued',
                workspacePath: path.join(process.env.HQ_ROOT, 'workers', 'lrq_aom03_agentos'),
              }),
              stderr: '',
            };
          }
          if (command === 'hq' && args[0] === 'dispatch') {
            return {
              stdout: JSON.stringify({
                launchRequestId: 'lrq_aom03_agentos',
                dispatchId: 'dispatch_aom03_agentos',
              }),
              stderr: '',
            };
          }
        ...
        spawnImpl: () => {
          spawnCalls += 1;
          return { pid: 7600, unref() {} };
        },
      });

      const dispatchCommand = commands.find((entry) => entry[0] === 'hq' && entry[1] === 'dispatch' && !['status', 'cancel'].includes(entry[2]));
      assert.equal(result.consumed, true);
...
test('dispatchRemediationViaHq carries workflow push provider requirement through agent-os payload', async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  const hqRoot = path.join(rootDir, 'agent-os-hq');
  mkdirSync(hqRoot, { recursive: true });
  const promptPath = path.join(rootDir, 'prompt.md');
  ...
  await withAppContractDispatchServer(async ({ requests }) => {
    ...
    const worker = await dispatchRemediationViaHq({
      ...
      repo: 'laceyenterprises/clio',
      prNumber: 83,
      branch: 'codex/fix-pr-83',
      promptPath,
      replyPath,
      launchRequestId: 'laceyenterprises__clio-pr-83-headabc123',
      jobId: 'laceyenterprises__clio-pr-83-headabc123',
    ...
    const dispatchRequest = requests.find((entry) => entry.url === '/v1/dispatch');
    assert.ok(dispatchRequest);
    assert.equal(dispatchRequest.body.credential_requirements.branch_push.provider, 'github-app-merge-agent');
    assert.equal(dispatchRequest.body.credential_requirements.branch_push.reason, 'workflow-file-remediation');
    assert.deepEqual(dispatchRequest.body.credential_requirements.branch_push.permissions, {
      contents: 'write',
      workflows: 'write',
    });
    assert.equal(
...
test('consumeNextFollowUpJob keeps native remediation on the bare spawner without legacy flag', async () => {
  ...
  await withOAuthTestEnv(rootDir, async () => {
    createFollowUpJob({
      ...
      prNumber: 77,
      reviewerModel: 'claude',
      linearTicketId: 'LAC-277',
      reviewBody: '## Summary\nKeep this native remediation bare.\n\n## Verdict\nRequest changes',
      reviewPostedAt: '2026-04-21T08:00:00.000Z',
      critical: false,
    });

    await withRemediationRouteEnv(rootDir, {
      AGENT_OS_ROLES_ADVERSARIAL_ORCHESTRATION_MODE: 'native',
      ADV_WITH_HQ_INTEGRATION: undefined,
    }, async () => {
      let spawnCalls = 0;
      const commands = [];
...
test('consumeNextFollowUpJob lets legacy HQ flag override native orchestration mode', async () => {
  const rootDir = mkdtempSync(path.join(tmpdir(), 'adversarial-review-'));
  ...
  await withOAuthTestEnv(rootDir, async () => {
    createFollowUpJob({
      ...
      prNumber: 78,
      reviewerModel: 'claude',
      linearTicketId: 'LAC-278',
      reviewBody: '## Summary\nNative mode with legacy HQ override.\n\n## Verdict\nRequest changes',
      reviewPostedAt: '2026-04-21T08:00:00.000Z',
      critical: false,
    });
    ...
    await withRemediationRouteEnv(rootDir, {
      ...
      const result = await consumeNextFollowUpJob({
        ...
        execFileImpl: async (command, args) => {
          ...
          if (command === 'gh' && args[0] === 'api' && /\/pulls\//.test(args[1])) {
            return {
              stdout: JSON.stringify({
                base: { ref: 'main' },
                head: { ref: 'codex/fix-pr-78', repo: { full_name: 'laceyenterprises/clio' } },
              }),
              stderr: '',
            };
          }
          if (command === 'hq' && args[0] === 'dispatch' && args[1] === 'status') {
            return {
              stdout: JSON.stringify({
                status: 'queued',
                workspacePath: path.join(process.env.HQ_ROOT, 'workers', 'lrq_aom03_override'),
              }),
              stderr: '',
            };
          }
          if (command === 'hq' && args[0] === 'dispatch') {
            return {
              stdout: JSON.stringify({
                launchRequestId: 'lrq_aom03_override',
                dispatchId: 'dispatch_aom03_override',
              }),
              stderr: '',
            };
          }
        ...
        spawnImpl: () => {
          throw new Error('legacy HQ override must not use the bare spawner');
        },
      });
...
test('resolveRemediationDispatchPathForJob keeps in-progress jobs on their claimed path across mode flips', () => {
  assert.equal(
    resolveRemediationDispatchPathForJob(
      { remediationPlan: { dispatchPath: 'bare' } },
      { AGENT_OS_ROLES_ADVERSARIAL_ORCHESTRATION_MODE: 'agentos' },
    ),
    'bare',
  );
  assert.equal(
    resolveRemediationDispatchPathForJob(
      { remediationPlan: { dispatchPath: 'hq' } },
      { AGENT_OS_ROLES_ADVERSARIAL_ORCHESTRATION_MODE: 'native' },
    ),
    'hq',
  );
