...
| `review:first_pass_ci_orphan` | open first-pass PR head has failed required checks, no review verdict, and no known live worker on that branch; first-pass review is not CI-gated, so this is additive to queue starvation rather than a replacement | ticket | required checks pass, a worker owns the branch, a review verdict posts, or the PR leaves the open population |
