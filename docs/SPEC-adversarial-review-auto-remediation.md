...
Within the review body's `## Verdict` section, the authoritative verdict is the last recognized verdict line. If the section contains both a blocking `Request changes` verdict line, including clause forms such as `Request changes: ...`, and a permissive `Comment only` or `Approved` line, the parser resolves conservatively to `Request changes`; explanatory prior-round resolution prose such as `Request changes ... resolved` does not trigger this override. If no line is recognized, the verdict remains malformed and the gate fails closed.
