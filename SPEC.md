...
\- Merge-agent dispatch precedence is: closed/merged PRs; explicit skip labels; active remediation; the normal verdict/mergeable/check/remediation gates and stale \`operator-approved\` diagnostics; a scoped \`merge-agent-requested\` override for bypassable gates; duplicate-dispatch protection; dispatch. A successful dispatch removes only the label that actually authorized that dispatch, preserving unused operator labels as audit trail.
