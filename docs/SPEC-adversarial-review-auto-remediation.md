...
5. **Final-pass-on-budget-exhausted (opt-in):** when `MERGE_AGENT_FINAL_PASS_ON_REQUEST_CHANGES=1` is set in the per-call env (typically on the follow-up daemon LaunchAgent) AND `remediationCurrentRound >= remediationMaxRounds` AND the verdict is still `request-changes` AND no scoped `operator-approved` label is present, the merge-agent is dispatched with the trigger `final-pass-on-budget-exhausted`. The merge-agent's own `comment_only_followups.py` sub-worker is then responsible for the final substance triage: apply trivial findings inline, defer non-trivial ones, refuse to merge if any blocker-class finding (data corruption, secret leakage, security regression, broken external contract) remains. The dispatched worker receives the trigger via the `MERGE_AGENT_DISPATCH_TRIGGER` env var (machine-readable) AND in the rendered prompt's `## Mode: final-pass-on-budget-exhausted` section (human/agent-readable), so the merge-agent's adapter and prompt can branch on dispatch mode without parsing markdown. The universal hard gates, the `mergeable === 'MERGEABLE'` requirement, and the `SUCCESS` check rollup requirement all still apply — failing CI or a conflicted PR still skips even with the flag enabled.
...
Without the final-pass path, every PR whose verdict never converges to `Comment only` halts at `max-rounds-reached` and waits for the operator. In practice the codex reviewer almost always returns `Request changes` because the reviewer prompt is adversarial by design; the lenient final-round addendum relaxes *categorization* but keeps the *verdict* at `Request changes` whenever any finding remains. Result: the auto-merge daemon has never auto-merged a single PR in the observed window leading up to 2026-05-14. The final-pass dispatch path closes that loop by giving the merge-agent itself the responsibility for the final substance check — the merge-agent's `comment_only_followups.py` is already designed for this triage, and it is the right place to decide whether reviewer findings warrant blocking a merge versus deferring follow-ups for a separate PR.

This is a behavioral expansion of the merge contract, gated off by default. Operators flip `MERGE_AGENT_FINAL_PASS_ON_REQUEST_CHANGES=1` on the follow-up daemon LaunchAgent to enable. Re-tune risk-class budgets independently if needed.
