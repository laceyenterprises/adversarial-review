...
- `merge-agent-skip`, `do-not-merge`, `no-merge-hold`, `duplicate-family-hold` — hard skips that even an operator-approved or merge-agent-requested label does not bypass. `merge-agent-stuck` is a hard skip by default, but a scoped current-head `merge-agent-requested` label may bypass it for explicit operator recovery. `duplicate-family-hold` is watcher-owned and clears after the duplicate-family census no longer sees an unresolved live unsuppressed family for the candidate, or after suppression/override evidence releases that candidate.
