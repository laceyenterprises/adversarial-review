...
- `review_pipeline_operational_blocker_stopped_rounds`: stopped remediation
  rounds that recorded operational blockers, grouped by blocker title/category.
- `review_pipeline_operational_blocker_oldest_age_seconds`: age of the oldest
  stopped remediation round that recorded an operational blocker.
...
| `review:operational_blocker_stopped_round` | at least one stopped remediation round recorded `operationalBlockers`; auth blockers use canonical `missing-auth` / `auth-failure` titles and may include a bounded rescue bundle under `$HQ_ROOT/remediation-rescue/`, falling back to ignored local `data/remediation-rescue/` when `HQ_ROOT` cannot be resolved | ticket | the stopped round is requeued, rescued/resumed, or aged out of the stopped queue/archive population; rescue bundles are best-effort retained for 7 days and capped at 256 MiB total |
