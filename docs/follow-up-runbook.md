...
For `review_status='failed'`, the watcher may auto-retry only when the row matches the bounded infrastructure-recovery classifier. The row must be rediscovered as a current open PR in an active repo, watcher drain must be inactive, active follow-up/backoff/merge-agent handoff gates must allow the spawn, memory admission must pass, routing-tier readiness must be healthy, and the dedicated infrastructure SQL compare-and-swap must promote the row to `reviewing`. The prior `failed_at` and `failure_message` remain intact until that claim succeeds. Infrastructure-class failures (`cascade`, including exhausted transient `gh pr diff` / GitHub GraphQL failures, `reviewer-timeout`, `launchctl-bootstrap`, reviewer-spawn `oauth-broken`, and `reviewer-command-failed` rows whose stored evidence starts with `[unknown] Command failed` or `[unknown] Command failed with code <n>`) use that dedicated claim to atomically increment `infra_auto_recover_attempts` only if the row is still `failed` with the same infrastructure class; once that counter reaches the cap, the row remains `failed` for operator inspection. The counter resets after a successful posted review or an intentional re-review re-arm. `forbidden-fallback`, unknown/non-infrastructure failures, inactive repos, PRs missing from GitHub discovery, closed or merged PRs, and rows skipped during watcher drain are not silently reset.
