...
- The original worker id is derived from the PR branch prefix only when the prefix matches a shipped worker-authored pattern (`codex-*`, `claude-code-*`, or `clio-agent-*`). Human branches like `paul/my-feature` do not opt into teardown and emit `merge_agent.branch_not_worker_authored` for operator visibility.
- If `HQ_ROOT/workers/<workerId>/workspace.json` is missing, the worker directory is gone, or the recorded worktree path no longer exists on disk, prep returns `decision: 'ready'` with reason `original-worker-already-torn-down`. This is the idempotent "already gone" exit.
...
- If the worker directory and worktree still exist, prep consults `worker_runs.status` from the session ledger. When the row is missing but the worktree is still present, prep returns `decision: 'deferred'` with reason `original-worker-run-row-missing-but-worktree-present`; ledger drift is not treated as branch freedom.
- Prep may tear down the original worker only when either (a) the current `worker_runs.status` is `succeeded`, re-checked immediately before the mutation, or (b) the PR lifecycle is already terminal (`closed`, `merged`, `merged-pending`, or `merged === true`). A non-terminal status logs `merge_agent.dispatch_deferred` with `reason: worker-run-status-<status>`; the same event is used for other defer reasons such as `missing-ledger-db` or `worker-run-lookup-failed`.
- The mutating teardown call honors Agent OS ownership boundaries. If `HQ_ROOT` is owned by a different uid than the watcher process, prep shells out as `sudo -A -u <hqRootOwner> env HQ_ROOT=<hqRoot> <HQ_BIN> worker tear-down <workerId>`; same-user ownership calls `hq worker tear-down <workerId>` directly with `HQ_ROOT` in the child env. Successful teardown logs `merge_agent.original_worker_tornDown` with the PR number, original worker id, worker status, and launch request id.
