...
If the loop hits that cap, the correct action depends on the operator's `MERGE_AGENT_FINAL_PASS_ON_REQUEST_CHANGES` setting on the follow-up daemon:

- **Flag OFF (legacy / OSS deployments):** After the cap is consumed, the next adversarial review pass uses the lenient final-round verdict-categorization addendum, but the automated merge gate halts: if any finding remains (blocking or non-blocking), the verdict stays `Request changes` and the system posts a public PR comment saying human intervention is required. A current scoped `operator-approved` label is the explicit human override for that head, not an automated relaxation.
- **Flag ON (deployment default on this host, see SPEC-adversarial-review-auto-remediation.md §Auto-Merge Convergence Loop):** After the cap is consumed and the verdict is still `Request changes`, the merge-agent is automatically dispatched with the `final-pass-on-budget-exhausted` trigger. The merge-agent's `comment_only_followups.py` sub-worker then **applies every actionable in-scope reviewer finding inline** — trivial polish and substantive non-trivial work alike — and the merge-agent triggers a fresh review pass when anything substantive lands. The sub-worker only records two non-apply outcomes:
  - `suggestions_unable_to_apply` for findings that genuinely cannot fit in this PR (multi-PR scope, conflicts with PR intent). These do NOT block the merge directly when code/config edits were also applied and force-pushed, because the new head can enter `awaiting-rereview` so the next reviewer evaluates both the edits and the punt. If no code/config edit was made, there is no new head SHA or durable same-head rereview request; the merge-agent must hard-refuse / operator-handoff instead of pretending a same-head rereview was queued.
  ...
  - A scoped `operator-approved` label still works as an explicit override and takes precedence; the final-pass path is for the case where no operator is available to look at the PR.

When the loop hits the cap, the first thing to check is which mode the daemon is in: `launchctl print gui/$(id -u)/<follow-up-daemon-label> | grep MERGE_AGENT_FINAL_PASS_ON_REQUEST_CHANGES`. If the flag is on, watch the `merge-agent-dispatches/` record and the merge-agent's own log to see how the triage step decided. If `suggestions_unable_to_apply` is non-empty alongside force-pushed edits, those items will be replayed against the next reviewer pass on the new head; if `suggestions_unable_to_apply` is non-empty without any applied edit, treat it as operator handoff because there is no durable same-head rereview trigger. If `blockers_observed` fired, inspect `.adversarial-follow-up/followups-reply.json` on the merge-worker workspace (NOT the receipt) for the redacted payload and decide whether to add `do-not-merge` / `merge-agent-skip` or to split the blocker into its own PR.
