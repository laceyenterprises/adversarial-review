...
- Watcher startup runs a stale-state reaper for offline-period recovery. It marks aged `reviewer_passes.status='running'` rows abandoned, releases stale AMA closer leases, immediately releases younger leases when the recorded watcher PID is provably dead, and unlinks syntactically corrupt/truncated AMA closer lease files as abandoned partial-failure state. Transient filesystem read errors on otherwise valid lease files are logged and left intact; the cursor advances so one persistent failure cannot blind later leases, then retries the failed name after a full lexical rotation. A lease that disappears after discovery is treated as a normal concurrent completion and does not halt the page. Closer-lease discovery scans the directory while retaining only bounded lexical candidate heaps, resumes after the last handled name, and admits at most `ADVERSARIAL_STALE_CLOSER_LEASE_ENTRY_SCAN_LIMIT` names (default `250`) and `ADVERSARIAL_STALE_CLOSER_LEASE_READ_LIMIT` JSON records (default `50`) per pass. The reaper writes `data/recovery-reaper/closer-lease-cursor.json` atomically after each page so read-capped leases remain next while persistent read or release failures are retried on wrap without starving the directory tail. This name-based continuation remains stable when owners concurrently delete earlier leases; names inserted before the cursor are picked up when the cursor wraps. When releasing a stale lease also needs to reset a transiently exhausted AMA closer budget, the reaper keeps the lease as the retry signal while continuing cursor progress. The sweep is fail-safe: errors are logged and must not prevent the watcher from polling.
