...
test('pollOnce keeps dag autowalk-on-merge retry as a single poll-level pass', () => {
  ...
  assert.ok(mergedSync < mergedMark, 'merged lifecycle state must commit after triage sync');
  assert.ok(closedSync < closedMark, 'closed lifecycle state must commit after triage sync');
