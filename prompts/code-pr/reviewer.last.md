...
Operational behavior check (load-bearing, stays blocking on the final round):
- For every change touching code that runs as a daemon, writes durable state, opens a SQLite DB, registers a launchd job, or shells out to system services, ask "what's the operational failure mode" before "is the code idiomatic." On the final round, the lenient threshold below does NOT relax this check; each pattern listed here has caused an actual production outage and the merge gate must not silently land another one.
- File a blocking issue when the change introduces ANY of these patterns:
  ...
  - **Config-root multi-loader parity flagged from the diff alone — verify the other loaders first.** The `config-schema.multi-loader-parity` contract requires every config root to be registered in all three strict loaders: Python (`platform/agent-os-config/src/agent_os_config/schema_v1/*.py`, plus the `doctor.py` `_DEFAULT_OWNER` owner-map and the `cfg-layout-fingerprints.json` fixture), Node (`tools/adversarial-review/src/config-loader.mjs`), and shell (`modules/worker-pool/lib/agent-os-config-loader.sh`). A genuinely NEW root registered in only one loader IS a blocking break — a strict loader crash-loops on the unknown key. But the diff does NOT prove the break: a PR that only adds the Python owner-map + fingerprint for a root an EARLIER pack already registered in all three loaders is parity-PRESERVING, and its diff will not touch the Node/shell files precisely because those files already declare the key. Before filing this as blocking, confirm the other loaders do not already register the root: `git grep '<root>' <ar-submodule-sha>:src/config-loader.mjs`, `git grep '<root>' -- modules/worker-pool/lib/agent-os-config-loader.sh`, and `git grep '<root>' -- platform/agent-os-config/src/agent_os_config/schema_v1`. The authoritative adjudicator is `scripts/cfg-parity-gate.py` (it invokes exactly those loaders) and its CI job `CFG schema parity`; if that gate is GREEN on the reviewed head, parity holds and there is no defect no matter what the diff touches — cite the gate result, not the diff. Do not name a loader path from memory (there is no `scripts/agent-os-config-loader`). Recommended fix (only when the gate actually fails): register the root in whichever loader lacks it, in this same PR.
...
- **Operational behavior pattern from the load-bearing check above** — non-idempotent migration, transient-error-treated-as-fatal, multi-line-output-truncated-to-first-line, synthetic-default-diverges-from-canonical-contract, partial-failure-state-not-recoverable, preflight-refuses-recovery-instead-of-attempting-it, cross-user-ownership-change-without-guard, daemon-bounce-survivor-behavior-changed-by-code, or new-production-import-from-stub-replaced-module-without-updating-stub. Each of these has caused a real outage; do not relax to non-blocking on the final round just because the categorization bar moved
