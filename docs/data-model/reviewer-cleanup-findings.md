...
is visible and the `reviewed_prs` row has safely moved from `reviewing` to
`posted`. The cleanup-finding store keeps that process evidence durable after
the row leaves the active `reviewing` population, so the watcher does not reduce
a still-live process group to a single log line.
