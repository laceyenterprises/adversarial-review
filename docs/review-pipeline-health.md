...
| `review:config_signature_drift` | a long-lived daemon's last successfully loaded config signature differs from disk for more than 10m, including failed reloads after the shared config cache has been reset | ticket | the daemon reloads the changed config successfully, the disk config is restored to the loaded signature, or the status file reports the daemon back in sync |
