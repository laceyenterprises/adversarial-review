...
- **Default ON (this deployment + code default):** After the cap is consumed and the verdict is still `Request changes`, the watcher first checks the latest structured `## Blocking issues` section. If the review is legacy/malformed and lacks the structured blocking section, it parks with `skip-blocking-findings-unknown` because blocker state is not knowable. When blocker state is known-zero, the merge-agent is automatically dispatched with the `final-pass-on-budget-exhausted` trigger plus `MERGE_AGENT_BLOCKING_FINDING_COUNT=0`. On that zero-blocker path, the merge-agent's `comment_only_followups.py` sub-worker **applies every actionable in-scope reviewer finding inline** — trivial polish and substantive non-trivial work alike. Light-to-medium fixes are pushed, checked, and merged without another review. Only major in-PR refactors trigger a fresh adversarial review pass. When blocker state is known-nonzero, the watcher permits exactly one automatic blocker-remediation dispatch per PR/head progression with the distinct trigger `final-pass-blocker-remediation`, `MERGE_AGENT_BLOCKING_FINDING_COUNT=<n>`, and `MERGE_AGENT_BLOCKER_REMEDIATION_REQUIRED=1`; that mode is the machine-readable no-merge discriminator for downstream code and adapters. The merge-agent applies the blocking findings inline, pushes the remediated head, and requests a fresh adversarial review (`reReview.requested=true` / `awaiting-rereview`) instead of merging its own blocker fixes.
  ...
  After HAM terminal remediation has been accepted and the daemon merge succeeds, reviewer-pass token recording and the succeeded AMA audit are the critical state updates. Hammer worker teardown is still attempted with bounded transient retries, but a teardown failure is logged and returned as non-fatal cleanup debt instead of aborting those post-merge audit writes. The same-head daemon merge handoff must pass the canonical live-gate shape into the merge predicate: current head, required checks, canonical `mergeable`, `mergeStateStatus`, `prState`, and an explicit `merged` boolean so recovery can distinguish an already-merged head from an open clean head.
