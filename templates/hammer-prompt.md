...
adversarial review submitted on commit `<<REVIEWED_SHA>>`. Treat its findings
...
```bash
gh pr view <<PR_URL>> --json number,headRefOid,state,isDraft,mergeable,mergeStateStatus,labels,statusCheckRollup,author,baseRefName > /tmp/ham-pr-post.json
...
its parent must be `<<REVIEWED_SHA>>`. If the PR head moved to a later non-HAM
...
- The remediation commit parent is `<<REVIEWED_SHA>>`.
...
```bash
gh pr view <<PR_URL>> --json number,headRefOid,state,isDraft,mergeable,mergeStateStatus,labels,statusCheckRollup,author,baseRefName > /tmp/ham-pr.json
gh pr view <<PR_URL>> --json reviews,comments > /tmp/ham-review-and-comments.json
...
- No merge on the old `<<REVIEWED_SHA>>` after remediation.
