...
for HAM_AUDIT_SHA_ATTEMPT in 1 2 3; do
  if POST_REMEDIATION_SHA=$(gh pr view https://github.com/acme/myrepo/pull/1234 --json headRefOid --jq '.headRefOid' 2> /tmp/ham-audit-pr-view.stderr) &&
...
for HAM_AUDIT_COMMENT_ATTEMPT in 1 2 3; do
  ...
  if GH_TOKEN="$MERGE_AGENT_GH_TOKEN" gh pr comment https://github.com/acme/myrepo/pull/1234 --body "$HAM_AUDIT_COMMENT_BODY"; then
    HAM_AUDIT_COMMENT_POSTED=1
    break
  fi
...
- No merging a branch that is `BEHIND` / not rebased onto the latest `main`; the
  ...
  before merge.
- No daemon handoff without holding the merge lease for `(acme/myrepo, base, PR 1234)`
