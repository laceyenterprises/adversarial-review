...
The liveness gate preserves the 2026-08-06 `worker_killed` invariant: a merged PR alone is not enough to delete a hammer worktree while the hammer may still be in its post-merge close sequence. Definitive active dispatch status defers; definitive terminal or absent status, and a readable manifest with no launch request id, permit immediate reaping. Transient or unreadable dispatch probes defer at first. REAPBLIND-01 bounds that unknown state with `AMA_CLOSER_WORKTREE_UNKNOWN_PROBE_LIMIT` (default `3`): after consecutive unknown dispatch probes, the reaper asks `lsof` whether any process has `cwd` at or below the worker directory. A negative process result is trusted only when the worker directory is owned by the daemon uid, the configured `AGENT_OS_WORKER_RUN_AS_USER` or `HQ_WORKER_RUN_AS_USER` is absent or names the daemon user, and the probe completes without diagnostics. Cross-uid ownership, a foreign configured run-as user, timeout, signal, buffer overflow, missing `lsof`, or permission diagnostics remain `unknown` and defer to the worker-pool orphan-reaper backstop rather than authorizing deletion. When a dispatch probe returned a pid, the `lsof` query is scoped with `-p <pid>`; otherwise the daemon performs one same-uid cwd scan per reap tick and reuses that snapshot for every escalating entry. `AMA_CLOSER_WORKTREE_PROCESS_PROBE_TIMEOUT_MS` (default `15000`) controls that escalation timeout. The cursor persists unknown-probe counters as `probeFailures`, prunes counters for workers no longer in the current worker discovery or older than `AMA_CLOSER_WORKTREE_PROBE_FAILURE_TTL_MS` (default one hour), caps the map with `AMA_CLOSER_WORKTREE_PROBE_FAILURE_MAX_ENTRIES` (default `512`), and writes cursor `schemaVersion: 2`.

The AMA closer provision path now also handles same-PR hammer collisions directly. If a branch-holder failure points at a same-PR `hammer-ama-pr-<samePR>-*` holder or a terminal coding-worker holder, the closer first attempts targeted `git worktree remove --force <path>` against that worker's `agent-os` checkout, then runs `hq worker tear-down --force` with bounded transient retries only after the git removal succeeds, and finally retries dispatch once. Keeping git cleanup first leaves `workspace.json` and run metadata available for the next preflight if git removal fails; if cleanup cannot clear the holder after the bounded retry window, the existing branch-holder debt accounting remains in force.

## Regression Coverage

Tests cover merged-PR reaping without touching open registered worktrees, half-registered and prunable cleanup, direct dispatch-liveness classification, direct process-cwd oracle classification, probe-failure cursor reclamation, and the #3064-style same-PR provision collision where the second hammer attempt tears down the first holder and provisions cleanly.
