...
The value is read at daemon startup. Invalid, missing, or non-positive values fall back to `1`, preserving the historical one-job behavior. Raising it lets a tick claim and spawn multiple pending jobs until `in-progress/` plus newly spawned jobs reaches the cap. Pending jobs for a PR that already has an in-progress remediation worker are left pending for a later tick; other PRs can still fill the remaining capacity.
