...
export async function load(url, context, nextLoad) {
  ...
  const simpleStubs = {
    ...
    'fixture:adversarial-gate-status': "function mergeableState(input = {}) { const mergeable = String(input.mergeable || '').trim().toUpperCase(); const mergeStateStatus = String(input.mergeStateStatus || '').trim().toUpperCase(); if (mergeable === 'MERGEABLE' || mergeable === 'CONFLICTING') return mergeable; if ((!mergeable || mergeable === 'UNKNOWN') && mergeStateStatus === 'CLEAN') return 'MERGEABLE'; return mergeable || mergeStateStatus; } export function deleteGateRecordsForPR() {} export async function buildAdversarialGateSnapshot(rootDir, args = {}) { return { reviewRow: args.reviewRow || null, latestJob: null, operatorApproval: null, labels: args.labels || [], headSha: args.headSha || null, settledReview: { verdict: '', remediationPending: false, reviewedHeadSha: null, blockingFindingState: 'unknown', blockingFindingCount: 0 }, requiresLiveHeadReviewReconciliation: false, mergeableState: mergeableState(args.mergeability) }; } export function pickAdversarialGateStatus() { return { state: 'pending', reason: 'fixture', context: 'fixture' }; } export async function projectAdversarialGateStatus() { return { decision: { state: 'pending', reason: 'fixture' } }; } export function resolveSettledReviewVerdict() { return { verdict: '', remediationPending: false }; }",
    ...
    'fixture:follow-up-merge-agent': "export const MERGE_AGENT_DISPATCHED_LABEL = 'merge-agent-dispatched'; export const MERGE_AGENT_REQUESTED_LABEL = 'merge-agent-requested'; export const MERGE_AGENT_DISPATCHED_LABEL_ADD_TRANSITION = 'dispatched-label-add'; export function classifyBlockingFindings() { return { count: 0, state: 'known' }; } export async function addMergeAgentDispatchedLabel() { return { added: false }; } export function buildMergeAgentDispatchJob() { return null; } export async function dispatchMergeAgentForPR() { return { dispatched: false }; } export function fetchMergeAgentCandidate() { return null; } export async function cancelMergeAgentDispatchOnMerge() { return { attempted: false, cancelled: false, labelRemoved: false }; } export function clearMergeAgentLifecycleCleanup() { return true; } export function listMergeAgentDispatches() { return []; } export function listMergeAgentLifecycleCleanups() { return []; } export async function isMergeAgentDispatchActiveForHead() { return { active: false, reason: 'fixture' }; } export async function pollFastMergeQueue() { return { processed: 0, merged: 0, blocked: 0, requeued_head_change: 0, requeued_veto: 0, skipped_still_pending: 0 }; } export function resolveFastMergePerPollCap() { return 5; } export function shouldUseReviewerTimeoutExhaustedMergeGate() { return false; } export function summarizeChecksConclusion() { return 'SUCCESS'; } export function updateMergeAgentLifecycleCleanup() { return {}; } export function upsertMergeAgentLifecycleCleanup() { return {}; } export function scanStuckMergeAgentDispatches() { return []; } export async function reconcileProactivePhantomHandoffs() { return { inspected: 0, graceStarted: 0, escalated: 0 }; } export function validateStartupMergeAgentConfig() {} export function isScopedMergeAgentRequest() { return false; }",
