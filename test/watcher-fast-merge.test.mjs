...
export async function load(url, context, nextLoad) {
  ...
  const simpleStubs = {
    ...
    'fixture:follow-up-jobs': "export const FOLLOW_UP_JOB_DIRS = { pending: 'pending', inProgress: 'in-progress', completed: 'completed', failed: 'failed', stopped: 'stopped', workspaces: 'workspaces', stoppedArchived: 'stopped-archived' }; export function classifyFollowUpCriticality() { return { critical: false, blockingFindingCount: 0, blockingFindingState: 'known', verdict: 'comment-only' }; } export function createFollowUpJob() { return { jobPath: '/tmp/watcher-fast-merge-follow-up.json' }; } export function listFollowUpJobsInDir() { return []; } export function listInProgressFollowUpJobs() { return []; } export function resolveRoundBudgetForJob() { return { roundBudget: 1, riskClass: 'medium' }; } export function summarizePRRemediationLedger() { return { completedRoundsForPR: 0, latestRiskClass: 'medium', latestMaxRounds: 1 }; } export const PUBLIC_REPLY_MAX_CHARS = 1200; export function detectPublicReplyNoiseSignal() { return null; } export function isActiveFollowUpJobStatus(status) { return ['pending','inProgress','in-progress','in_progress'].includes(status); } export function markFollowUpJobStopped() { return { status: 'stopped' }; } export function requeueFollowUpJobForNextRound() { return { requeued: false }; } export function requeueInProgressFollowUpJobForRetry() { return { requeued: false }; } export function writeFollowUpJob() {}",
    'fixture:duplicate-family-state': "export function duplicateFamilySchemaTableNames() { return []; } export function ensureDuplicateFamilySchema() {} export async function runDuplicateFamilyCensusForWatcher() { return { families: [], familyIds: [] }; } export async function reconcileDuplicateFamilyLabels() { return { inspected: 0, changed: 0 }; }",
    'fixture:remediation-prompt': "export function followUpJobRepoPrKey(job) { return String(job?.repo || '').toLowerCase() + '#' + (job?.prNumber || ''); }",
    'fixture:follow-up-merge-agent': "export const MERGE_AGENT_DISPATCHED_LABEL_ADD_TRANSITION = 'dispatched-label-add'; export function classifyBlockingFindings() { return { count: 0, state: 'known' }; } export async function addMergeAgentDispatchedLabel() { return { added: true }; } export function buildMergeAgentDispatchJob() { return null; } export async function dispatchMergeAgentForPR() { return { dispatched: false }; } export function fetchMergeAgentCandidate() { return null; } export async function cancelMergeAgentDispatchOnMerge() { return { attempted: false, cancelled: false, labelRemoved: false }; } export function clearMergeAgentLifecycleCleanup() { return true; } export function listMergeAgentDispatches() { return []; } export function listMergeAgentLifecycleCleanups() { return []; } export async function isMergeAgentDispatchActiveForHead() { return { active: false, reason: 'fixture' }; } export async function pollFastMergeQueue() { return { processed: 0, merged: 0, blocked: 0, requeued_head_change: 0, requeued_veto: 0, requeued_duplicate_family_hold: 0, skipped_still_pending: 0 }; } export function resolveFastMergePerPollCap() { return 5; } export function shouldUseReviewerTimeoutExhaustedMergeGate() { return false; } export function summarizeChecksConclusion() { return 'SUCCESS'; } export function updateMergeAgentLifecycleCleanup() { return {}; } export function upsertMergeAgentLifecycleCleanup() { return {}; } export function scanStuckMergeAgentDispatches() { return []; } export async function reconcileProactivePhantomHandoffs() { return { inspected: 0, graceStarted: 0, escalated: 0 }; } export function validateStartupMergeAgentConfig() {} export function isScopedMergeAgentRequest() { return false; }",
...
test('fast-merge watcher: duplicate-family-hold cannot be bypassed by skip label', () => {
  const labels = [{ name: 'fast-merge:docs' }, { name: 'duplicate-family-hold' }];
  const summary = runWatcherScenario({
    subjects: [subject(822, { labels })],
    labels: { 822: labels },
    timeline: { 822: [timelineLabel('fast-merge:docs', '2026-05-20T12:00:05.000Z')] },
  }, { skipEnabled: true });
  assert.notEqual(summary.rows[0].review_status, 'fast_merge_skipped');
  assert.equal(summary.rows[0].fast_merge_authorized_head_sha, null);
});
