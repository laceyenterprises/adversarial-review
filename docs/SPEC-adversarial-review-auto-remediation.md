...
- `diagnose-stuck-rereview --apply` is a daemon watchdog for rows that look abandoned after a durable worker rereview request. It is not an operator override: it must not clear reviewer failure evidence or retry counters, must not satisfy the explicit `retrigger-review:` bypass predicate, and must preserve both `rereview_requested_at` and `rereview_reason` as the original FIFO/latency clock and load-bearing marker. The daemon step must be disarmable through `ADVERSARIAL_REVIEW_STUCK_REREVIEW_APPLY=0` (or equivalent false-ish values) without introducing a config-loader parity surface. On an open pending row without `failed_at`, it may only request a `stuck-rereview-watchdog` watcher wake for the affected repo/PR/head as the retry nudge. It caches follow-up-job bucket listings once per invocation rather than scanning every bucket once per candidate row. Terminal-PR protection remains in the watcher's authoritative-live claim guard; a stale-open mirror row may consume bounded wake/backoff budget here, but must not cause reviewer spawn. Repeated wakes are capped and backed off per `(repo, pr, head)`, state entries older than seven days are pruned on write, cap exhaustion is surfaced as a skipped row rather than an apply failure so capped rows cannot consume the apply window forever, and a wake-write failure after a successful eligibility check still consumes backoff budget.
