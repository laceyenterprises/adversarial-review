...
function isWatcherRetryContractChange(lines) {
  return lines.some((line) => {
    ...
    return /infra_auto_recover_attempts|INFRA_AUTO_RECOVER_CAP|stmtMark(?:InfraAutoRecovery)?AttemptStarted|review_status\s*=\s*'failed'|pending-upstream|failed-orphan|reviewer-timeout|launchctl-bootstrap|oauth-broken|quota-exhausted|reviewer-command-failed/.test(line);
  });
}
