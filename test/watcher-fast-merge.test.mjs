...
export async function load(url, context, nextLoad) {
  ...
  const simpleStubs = {
    'fixture:routing': "export function routeSubject(subject) { return String(subject.title || '').startsWith('[codex]') || String(subject.title || '').startsWith('[claude-code]') ? { reviewerModel: 'claude', botTokenEnv: 'CLAUDE_TOKEN', tag: 'codex' } : null; } export function defaultReviewerRouteFromEnv() { return null; } export function describeCrossModelReviewWaiver() { return null; } export function isCrossModelReviewWaived() { return false; }",
