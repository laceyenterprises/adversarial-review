...
The harness Claude reviewer additionally arms a **first-output** deadline, `reviewer.first_output_timeout_ms`. This is not the no-output progress watchdog and does not contradict the paragraph above: `firstOutputTimeout` in `spawnCapturedProcessGroup` is armed once before the child runs and cleared permanently on the first byte of stdout or stderr — it is never re-armed, so it bounds a wedged launch without capping the quiet compute of a healthy non-streaming turn. Implementing this deadline with the rolling `progressTimeout` knob instead would cap a healthy review at that value and is a contract violation. A first-output kill raises `err.firstOutputTimedOut` (reason prefix `no first output for`), distinct from `err.progressTimedOut` (`no output for`), and is retried once in-process under the same reviewer pass.
