...
function buildWatcherLoaderSource(scenario) {
  ...
  const stubs = {
    ...
    [fileUrl('src', 'role-registry.mjs')]: 'fixture:role-registry',
    [fileUrl('src', 'adapters', 'comms', 'github-pr-comments', 'index.mjs')]: 'fixture:github-pr-comments',
...
export async function load(url, context, nextLoad) {
  if (url === 'fixture:review-state') {
    return {
      ...
      source: ${JSON.stringify(`
        ...
        // sweep-facing surface: a production module that joins this loader's
        // graph must never die on
        // "does not provide an export named 'resolvePRLifecycle'".
        // null == "learned nothing", which the sweep treats as fail-closed.
        export async function resolvePRLifecycle() { return null; }
      `)}
    };
  }
  ...
  const simpleStubs = {
    ...
    'fixture:config-loader': "export class AgentOSConfigError extends Error {} export const ENUM_ROLES_ADVERSARIAL_ORCHESTRATION_MODE = ['native', 'agentos']; export function getConfig(_key, fallback = null) { return fallback; } export function loadConfigCached() { return {}; }",
    ...
    'fixture:reviewer-runtime': "export function createReviewerRuntimeAdapterForDomain() { return { spawnReviewer: async () => ({ ok: true }), cancel: async () => {}, reattach: async () => ({}) }; } export function createReviewerRuntimeAdapterByName() { return createReviewerRuntimeAdapterForDomain(); } export function loadDomainConfig() { return {}; } export async function recoverReviewerRunRecords() { return { recovered: 0, failed: 0 }; }",
    ...
    'fixture:follow-up-jobs': "export const FOLLOW_UP_JOB_DIRS = { pending: 'pending', inProgress: 'in-progress', completed: 'completed', failed: 'failed', stopped: 'stopped', workspaces: 'workspaces', stoppedArchived: 'stopped-archived' }; export function classifyFollowUpCriticality() { return { critical: false, blockingFindingCount: 0, blockingFindingState: 'known', verdict: 'comment-only' }; } export function createFollowUpJob() { return { jobPath: '/tmp/github-api-fixture-follow-up-job.json' }; } export function listFollowUpJobsInDir() { return []; } export function listInProgressFollowUpJobs() { return []; } export function markFollowUpJobCompleted() { return { status: 'completed' }; } export function resolveRoundBudgetForJob() { return { roundBudget: 1, riskClass: 'medium' }; } export function summarizePRRemediationLedger() { return { completedRoundsForPR: 0, latestRiskClass: 'medium', latestMaxRounds: 1 }; } export function isActiveFollowUpJobStatus() { return false; } export function markFollowUpJobStopped() { return { status: 'stopped' }; } export function requeueFollowUpJobForNextRound() { return { requeued: false }; } export function requeueInProgressFollowUpJobForRetry() { return { requeued: false }; } export function writeFollowUpJob() {}",
    'fixture:remediation-prompt': "export function followUpJobRepoPrKey(job) { return String(job?.repo || '').toLowerCase() + '#' + (job?.prNumber || ''); }",
    'fixture:follow-up-merge-agent': "export const MERGE_AGENT_DISPATCHED_LABEL = 'merge-agent-dispatched'; export const MERGE_AGENT_REQUESTED_LABEL = 'merge-agent-requested'; export const MERGE_AGENT_DISPATCHED_LABEL_ADD_TRANSITION = 'dispatched-label-add'; export function classifyBlockingFindings() { return { count: 0, state: 'known' }; } export async function addMergeAgentDispatchedLabel() { return { added: false }; } export function buildMergeAgentDispatchJob() { return null; } export async function dispatchMergeAgentForPR() { return { dispatched: false }; } export function fetchMergeAgentCandidate() { return null; } export function pickMergeAgentDispatch() { return 'skip-fixture'; } export async function cancelMergeAgentDispatchOnMerge() { return { attempted: false, cancelled: false, labelRemoved: false }; } export function clearMergeAgentLifecycleCleanup() { return true; } export function listMergeAgentDispatches() { return []; } export function listMergeAgentLifecycleCleanups() { return []; } export async function isMergeAgentDispatchActiveForHead() { return { active: false, reason: 'fixture' }; } export async function pollFastMergeQueue() { return { processed: 0, merged: 0, blocked: 0, requeued_head_change: 0, requeued_veto: 0, skipped_still_pending: 0 }; } export function resolveFastMergePerPollCap() { return 5; } export function shouldUseReviewerTimeoutExhaustedMergeGate() { return false; } export function summarizeChecksConclusion() { return 'SUCCESS'; } export function updateMergeAgentLifecycleCleanup() { return {}; } export function upsertMergeAgentLifecycleCleanup() { return {}; } export function scanStuckMergeAgentDispatches() { return []; } export async function reconcileProactivePhantomHandoffs() { return { inspected: 0, graceStarted: 0, escalated: 0 }; } export function validateStartupMergeAgentConfig() {} export function isScopedMergeAgentRequest() { return false; }",
    ...
    'fixture:reviewer-cascade': "export const CASCADE_FAILURE_CAP = 3; export function clearCascadeState() {} export function clearReviewerCredentialOutage() { return null; } export function formatTransientFailureBreakdown() { return ''; } export function hasOperatorDecisionRequiredAlerted() { return false; } export function markCascadeCapExhaustedAlerted() { return { marked: true, state: {} }; } export function markOperatorDecisionRequiredAlerted() { return { marked: true, state: {} }; } export function readCascadeState() { return null; } export function recordCascadeFailure() { return { consecutiveTransientFailures: 1, transientFailureBreakdown: {}, backoffMinutes: 1 }; } export function recordReviewerCredentialFailure() { return { active: false }; } export function shouldBackoffReviewerSpawn() { return { shouldBackoff: false }; } export function shouldPauseReviewerModel() { return { paused: false }; }",
    ...
    'fixture:reviewer-lease': "export const DEFAULT_REVIEWER_LEASE_RECOVERY_MAX_ATTEMPTS = 3; export function computeReviewerLeaseExpiryAt() { return null; } export function isReviewerLeaseExpired() { return false; } export function resolveReviewerLeaseRecoveryEnabled() { return false; }",
    'fixture:reviewer-pass-tokens': "export function beginReviewerPass() { return { ok: true }; } export function completeReviewerPass() { return { ok: true }; } export function nextReviewerPassAttemptNumber(_rootDir, { fallbackReviewAttempts = 0 } = {}) { return Number(fallbackReviewAttempts || 0) + 1; } export function normalizeReviewerClass(value) { const text = String(value || '').toLowerCase(); if (text.includes('codex') || text.includes('gpt')) return 'codex'; if (text.includes('gemini') || text.includes('antigravity') || text.includes('agy')) return 'gemini'; return 'claude'; } export function readBestReviewerEvidenceTokenUsage() { return null; } export function readWorkerRunTokenUsageResult() { return { ok: false, reason: 'missing-worker-run-row' }; } export function tagTokenUsage(tokenUsage) { return tokenUsage || null; } export function writeReviewerTokenUsageArtifact() { return null; }",
    ...
    'fixture:session-ledger-read-adapter': "export function readBuildCompletionProducerEvidence() { return { ok: false, reason: 'missing-build-completion-producer-evidence' }; } export function readBuildCompletionSignalForPr() { return { ok: false, reason: 'missing-build-completion-signal' }; } export function readLatestWorkerRunStatusFromLedger() { return null; } export function resolveSessionLedgerReadTarget() { return null; }",
    ...
    'fixture:watcher-reviewer-pool': "export function compareReviewerDispatchCandidates() { return 0; } export function countActiveReviewerSpawnsByModel() { return new Map(); } export function createDetachedReviewerDispatchTracker() { return { activeCounts: () => new Map(), track: () => {} }; } export function createReviewerLaneState() { return {}; } export function createReviewerMemoryAdmissionSampler() { return { sample: async () => ({ admit: true }) }; } export function reserveReviewerMemoryAdmission() { return () => {}; } export function reviewerDispatchIsFirstPass() { return true; } export async function fetchGeminiCredentialConcurrency() { return null; } export function resolveFirstPassReviewerPoolConfig() { return { enabled: false }; } export function resolveReviewLaneConfig() { return { firstPassBurstLimit: 2 }; } export function resolveReviewerMemoryPressureConfig() { return {}; } export async function runBoundedReviewerDispatchQueue() { return { dispatched: 0, skipped: 0 }; } export function sortReviewerDispatchCandidates(items) { return items; } export function reviewerDispatchPassKind() { return 'first-pass'; }",
