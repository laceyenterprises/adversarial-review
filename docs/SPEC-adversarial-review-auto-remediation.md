...
- Operational lookup failures such as `missing-ledger-db`, `better-sqlite3-unavailable`, `worker-run-lookup-failed`, `worker-run-lookup-threw`, and `missing-launch-request-id` become explicit skipped-dispatch records instead of unbounded silent deferrals. `better-sqlite3-unavailable` also logs a loud dependency error.
...
- `merge-agent-skip`, `do-not-merge` — hard skips that even an operator-approved or merge-agent-requested label does not bypass. `merge-agent-stuck` is a hard skip by default, but a scoped current-head `merge-agent-requested` label may bypass it for explicit operator recovery.
