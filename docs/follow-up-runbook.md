...
- Until `LAC-358` is explicitly reverted, **all auto-remediation routes through the codex worker class regardless of the original PR's `builderTag`**. The durable `builderTag` is still preserved on the job for git attribution and history, but worker-class selection is intentionally hard-switched to codex per `feedback_prefer_codex_for_heavy_work.md` while claude-code remains unsuitable for unattended heavy remediation.
