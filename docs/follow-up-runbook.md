...
- Handoff wake marker files under `data/handoff-wake/` are a best-effort latency optimization, not the correctness path. The native writer path is allowed only when the caller UID matches the canonical owner of the existing wake directory, or the tool root before that directory exists. Cross-user callers must route marker creation through `sudo -A -H -u <owner>` so they do not create or re-parent daemon-owned coordination state under a worker UID. If owner resolution or sudo handoff fails, the signal is missed and the daemon falls back to its scheduled timer.
