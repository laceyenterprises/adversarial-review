...
Leave `ADVERSARIAL_REVIEW_DEFAULT_REVIEWER` unset for the normal reviewer routing. The base opposite-agent route is `[codex]` → Claude, `[claude-code]` → Codex, `[clio-agent]` → Claude because Clio dispatches Codex-family writers, and `[gemini]` / `[pi]` / `[opencode]` / `[hermes]` → Codex. The exported route helpers and watcher then apply `reviewer.gemini.mode` (default `always-on`) through the same effective-route helper: `[codex]`, `[claude-code]`, and `[clio-agent]` first-pass reviews go to Gemini with `GH_GEMINI_REVIEWER_TOKEN`; `[gemini]`, `[pi]`, `[opencode]`, and `[hermes]` keep Codex with `GH_CODEX_REVIEWER_TOKEN`. Set `reviewer.gemini.mode=fallback` to use Gemini only while the base reviewer is quota-capped, or `off` to preserve the base route. Gemini is never allowed to review `[gemini]` PRs; even an operator Gemini pin is stripped back to Codex for that builder class. Set `ADVERSARIAL_REVIEW_DEFAULT_REVIEWER` to `codex` to lock Claude out of first-pass reviews, to `claude` to force Claude reviews for every supported builder class, or to `gemini` to force the native Gemini reviewer runtime where the Gemini-on-Gemini hard guard permits it. The forced reviewer also overrides the Gemini default layer and selects the matching reviewer bot token (`GH_CODEX_REVIEWER_TOKEN`, `GH_CLAUDE_REVIEWER_TOKEN`, or `GH_GEMINI_REVIEWER_TOKEN`). The watcher validates this knob at startup and exits non-zero on invalid values; under launchd that means a typo in this optional knob intentionally crash-loops the watcher until the env is fixed, with a `FATAL config: ADVERSARIAL_REVIEW_DEFAULT_REVIEWER...` banner in stderr identifying the cause. If the pin deliberately causes same-family review outside the Gemini hard guard, the watcher logs a waiver and the posted review body includes a cross-model-waiver note for auditability. That waiver is audit-only and does not alter the merge gate; a same-family `Comment only` review can still satisfy the automated gate unless an operator or branch-protection policy adds a separate human check.
...
AMA closer duplicate-dispatch protection is keyed to the audit ledger and the same-head lease, not just the HQ worker exit status. A ledger `succeeded` outcome remains sticky authority that the PR closed and suppresses any later `hq dispatch status` probe. If the HQ dispatch reports `succeeded` before a terminal audit outcome is readable, the watcher holds ownership for that tick instead of immediately deleting the lease and launching another closer. If the ledger already shows a non-merged terminal outcome such as `deferred`, `failed-without-merge`, or `superseded`, the watcher does not delete a terminal closer lease merely because the old HQ dispatch status is retryable; same-head relaunches stay behind the dispatch record's bounded retry budget and live/terminal lease protection.
...
For `review_status='failed'`, the watcher may auto-retry only when the row matches the bounded infrastructure-recovery classifier. The row must be rediscovered as a current open PR in an active repo, watcher drain must be inactive, active follow-up/backoff/merge-agent handoff gates must allow the spawn, memory admission must pass, routing-tier readiness must be healthy, and the dedicated infrastructure SQL compare-and-swap must promote the row to `reviewing`. The prior `failed_at` and `failure_message` remain intact until that claim succeeds. Infrastructure-class failures (`cascade`, `reviewer-timeout`, `launchctl-bootstrap`, and reviewer-spawn `oauth-broken`) use that dedicated claim to atomically increment `infra_auto_recover_attempts` only if the row is still `failed` with the same infrastructure class; once that counter reaches the cap, the row remains `failed` for operator inspection. The counter resets after a successful posted review or an intentional re-review re-arm. `forbidden-fallback`, unknown/non-infrastructure failures, inactive repos, PRs missing from GitHub discovery, closed or merged PRs, and rows skipped during watcher drain are not silently reset.
