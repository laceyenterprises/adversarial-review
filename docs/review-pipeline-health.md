...
| `review:reviewer_model_silent` | a configured reviewer class with a previous genuine first-pass/rereview comment has not posted again past the larger of the 24h floor and that class's recent p75 post cadence, while at least one pass for that class started after that comment inside the activity lookback | ticket | the model posts another review inside its cadence-derived silence threshold or no recent started-pass demand remains after its last posted review |
...
- `ADVERSARIAL_REVIEW_PIPELINE_HEALTH_REVIEWER_SILENCE_THRESHOLD_MS`
  ...
  cadence, not a flat alarm threshold; cadence is considered only with at
  least five intervals and is capped at 2x this floor)
- `ADVERSARIAL_REVIEW_PIPELINE_HEALTH_REVIEWER_ACTIVITY_LOOKBACK_MS`
  (default `604800000`; bounds recent started-pass demand and cadence samples,
  not the last-post lookup used to decide whether a previously-active class has
  gone silent)
