...
Fresh transient reviewer failures (`cascade`, `provider-overloaded`, `reviewer-timeout`, `launchctl-bootstrap`, and `daemon-bounce`) settle directly to `review_status='pending-upstream'`, increment `infra_auto_recover_attempts` immediately, and wait for reviewer-lane/routing-tier recovery rather than first becoming `failed`. This is the documented path for those classes; they intentionally bypass the `failed` SQL compare-and-swap claim.
