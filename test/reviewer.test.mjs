...
test('postGitHubReviewWithCapture audits primary dismissal when cross-family dismissal fails', async () => {
  const calls = [];
  const logs = [];
  const warnings = [];
  const rootDir = mkdtempSync(join(tmpdir(), 'review-cross-family-dismissal-failure-'));
  mkdirSync(join(rootDir, 'data'), { recursive: true });
  const reviewBody = [
    '## Summary',
    'Clean after remediation.',
  ...
  try {
    await withEnvAsync({
      ...
      execFileImpl: async (command, args, options = {}) => {
        ...
        if (joined.includes('/reviews/9201/dismissals')) {
          return { stdout: '{}' };
        }
        if (joined.includes('/reviews/9202/dismissals')) {
          const err = new Error('fixture cross-family dismissal outage');
          err.status = 502;
          throw err;
        }
        if (joined.includes('/pulls/42/reviews?')) {
          return {
            stdout:
              ...
              JSON.stringify([
                {
                  id: 9201,
                  user: { login: 'lacey-gemini-reviewer[bot]' },
                  body: 'same-family block',
                  state: 'CHANGES_REQUESTED',
                  submitted_at: '2026-08-09T10:00:00Z',
                  commit_id: 'reviewed-head-sha',
                },
    ...
    const dismissalCalls = calls.filter((call) => call.args.join(' ').includes('/dismissals'));
    assert.equal(dismissalCalls.length, 2);
    assert.match(dismissalCalls[0].args.join(' '), /reviews\/9201\/dismissals/);
    assert.match(dismissalCalls[1].args.join(' '), /reviews\/9202\/dismissals/);
    assert.ok(!calls.some((call) => call.args.join(' ').includes('/reviews/9203/dismissals')));
    assert.match(warnings.join('\n'), /fixture cross-family dismissal outage/);
    const event = logs
      .map((line) => JSON.parse(line))
      .find((entry) => entry.event === 'reviewer.stale_request_changes.dismissal');
    assert.equal(event.ok, false);
    assert.equal(event.dismissalPhase, 'cross-family');
    assert.deepEqual(event.dismissed, ['9201']);
    assert.equal(event.attempted, 2);
    assert.equal(event.crossFamilyAttempted, 1);
    assert.deepEqual(event.crossFamilyDismissed, []);
    assert.equal(event.reviewId, '9202');
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
});
...
test('postGitHubReview reconciles ambiguous exact-head writes across slurped review pages', async () => {
  const calls = [];
  const result = await postGitHubReview(
    'laceyenterprises/demo',
    42,
    'review body on page two',
    'GH_CODEX_REVIEWER_TOKEN',
    async (command, args, options = {}) => {
      calls.push({ command, args, options });
      const isPost = args.includes('--method') && args.includes('POST');
          ...
          [
            {
              ...
              user: { login: 'lacey-codex-reviewer[bot]' },
              submitted_at: '2026-09-10T17:30:00Z',
            },
          ],
          [
            {
  ...
  assert.equal(calls.filter((call) => call.args.includes('--method') && call.args.includes('POST')).length, 1);
  const reconcileCalls = calls.filter((call) => call.args.includes('--paginate'));
  assert.equal(reconcileCalls.length, 1);
  assert.deepEqual(reconcileCalls[0].args, [
    'api',
    'repos/laceyenterprises/demo/pulls/42/reviews',
    '--paginate',
    '--slurp',
