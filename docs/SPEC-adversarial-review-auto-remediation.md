...
- Operational lookup failures such as `missing-ledger-db`, `better-sqlite3-unavailable`, `worker-run-lookup-failed`, `worker-run-lookup-threw`, and `missing-launch-request-id` become explicit skipped-dispatch records instead of unbounded silent deferrals. `better-sqlite3-unavailable` also logs a loud dependency error. Final-pass convergence parks caused by standing blockers (`skip-blockers-present`) or legacy/unknown blocker state (`skip-blocking-findings-unknown`) write the same durable skipped-dispatch record under `data/follow-up-jobs/merge-agent-skips/` with the blocker count/state.
