...
| `review:reviewer_model_silent` | a reviewer model that previously posted a review has not posted again for 24h; the snapshot carries the 7d activity-lookback tuning value for operator context | ticket | the model posts another review, moving its latest posted timestamp below the silence threshold |
