...
- `diagnose-stuck-rereview --apply` is an automation watchdog, not an operator retrigger surface. The follow-up daemon can disarm it immediately with `ADVERSARIAL_REVIEW_STUCK_REREVIEW_APPLY=0` (also accepts `false`, `off`, `no`, or `disabled`) without editing `config.yaml` or touching the strict multi-loader surface. It scans pending rows with aged `rereview_requested_at`, caches the follow-up-job bucket listings once per invocation, then preserves that original FIFO/latency timestamp and the existing `rereview_reason` marker byte-for-byte while writing a `stuck-rereview-watchdog` watcher wake for the affected repo/PR/head. It does not clear `review_attempts`, `last_attempted_at`, `failed_at`, `failure_message`, `reviewer_head_sha`, or retry counters, and rows carrying `failed_at` stay visible in read-only diagnostics while the apply path skips them for terminal-failure retry/finalization. The candidate query still trusts the local `reviewed_prs.pr_state='open'` mirror; terminal-PR safety is delegated to the watcher's live PR-state claim guard, so this path may burn a bounded wake/backoff slot for a stale-open terminal row but must not spawn a reviewer for it. Re-arms are tracked in `data/follow-up-jobs/stuck-rereview-watchdog.json` by `(repo, pr, head)`, use exponential backoff from the stuck threshold, record the original request time, prune entries whose last successful apply is older than seven days whenever the state file is rewritten, and stop after three attempts; cap exhaustion is reported as a skipped row rather than an apply failure so a capped row cannot consume the window forever. If the row eligibility check succeeds but writing the wake fails, the same state entry/backoff is still recorded so a persistent wake-file failure cannot bypass the cap.
