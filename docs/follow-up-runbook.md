...
Fresh transient reviewer failures (`cascade`, `oauth-broken`, `provider-overloaded`, `reviewer-timeout`, `launchctl-bootstrap`, and `daemon-bounce`) settle directly to `review_status='pending-upstream'`, increment `infra_auto_recover_attempts` immediately, and wait for reviewer-lane/routing-tier recovery rather than first becoming `failed`. This is the documented path for those classes; they intentionally bypass the `failed` SQL compare-and-swap claim. Repeated `oauth-broken` failures across distinct PRs also open a model-scoped outage file under `data/reviewer-credential-outages/`; the watcher preflight holds further spawns for that model until the file's `nextProbeAt`, then records a fresh probe reservation and lets one normal reviewer attempt through as a recovery probe. Success clears the outage and re-arms rows parked with the outage-transient marker; another OAuth failure records fresh evidence while preserving the reserved retry window when it is still in the future.
